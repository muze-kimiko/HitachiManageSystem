/*
 *  Licensed Materials - Property of IBM
 *  5725-I43 (C) Copyright IBM Corp. 2011, 2013. All Rights Reserved.
 *  US Government Users Restricted Rights - Use, duplication or
 *  disclosure restricted by GSA ADP Schedule Contract with IBM Corp.
 */
var path='eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA';

/**
 *  WL.Server.invokeHttp(parameters) accepts the following json object as an argument:
 *  
 *  {
 *  	// Mandatory 
 *  	method : 'get' , 'post', 'delete' , 'put' or 'head' 
 *  	path: value,
 *
 *  	// Optional
 *  	returnedContentType: any known mime-type or one of "json", "css", "csv", "plain", "xml", "html"
 *  	returnedContentEncoding : 'encoding',
 *  	parameters: {name1: value1, ... },
 *  	headers: {name1: value1, ... },
 *  	cookies: {name1: value1, ... },
 *  	body: {
 *  		contentType: 'text/xml; charset=utf-8' or similar value,
 *  		content: stringValue
 *  	},
 *  	transformation: {
 *  		type: 'default', or 'xslFile',
 *  		xslFile: fileName
 *  	}
 *  }
 */

/**
 * @param tag
 *            must be either MobileFirst_Platform or MobileFirst_Playground
 * @returns json list of items
 */
function getFeed(tag) {
	path = getPath(tag);

	var input = {
	    method : 'get',
	    returnedContentType : 'xml',
	    path : path
	};


	return WL.Server.invokeHttp(input);
}
/**
 *
 * @param tag
 *            must be either MobileFirst_Platform or MobileFirst_Playground
 * @returns json list of items
 */
function getFeedFiltered(tag) {
	path = getPath(tag);

	var input = {
	    method : 'get',
	    returnedContentType : 'xml',
	    path : path,
	    transformation : {
		    type : 'xslFile',
		    xslFile : 'filtered.xsl'
	    }
	};

	return WL.Server.invokeHttp(input);
}



function getPath(tag) {
	var path;
	if (tag === undefined || tag === '') {
		path = '/feed';
	}else {
		path = '/tag/' + tag + '/feed';
	}
	return 'mobilefirstplatform' + path;
}


//按条件查询保障表
function getBZMeasureQuery(param){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELBZMeasure">'+
		   		'<soapenv:Header/>'+
				'<soapenv:Body>'+
	      			'<hel:BZMeasureQuery_Input>'+
	         			'<hel:SearchSpec>'+param.SearchSpec+'</hel:SearchSpec>'+
	      				'<hel:ViewMode>'+param.ViewMode+'</hel:ViewMode>'+
	         		'</hel:BZMeasureQuery_Input>'+
	         	'</soapenv:Body>'+
	         	'</soapenv:Envelope>';
	
	WL.Logger.error("按条件查询保障表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELBZMeasure:BZMeasureQuery"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//按ID查询保障表  
//功能和"按条件查询保障表"是一样的 条件来源于上面方法获取的数据 Id 字段
function getBZMeasureQueryById(Id){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELBZMeasure">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:BZMeasureQueryById_Input>'+
	         '<hel:PrimaryRowId>'+Id+'</hel:PrimaryRowId>'+
	      '</hel:BZMeasureQueryById_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("按ID查询保障表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELBZMeasure:BZMeasureQueryById"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//插入或更新保障表
function getBZMeasureSynchronize(param){
	//WL.Logger.error("测试:"+param.LSYLWT[0].Id);
	//WL.Logger.error("测试2:"+param.LSYLWT.length);
	/*var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:meas="http://siebel.com/PDAMainPlan/MeasureItemAttachment" xmlns:hel="http://www.siebel.com/xml/HEL%20PDA%20Maintaining%20Plan%20Measure%20Item%20Attachment%20IO">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<meas:AttachmentSynchronize_Input>'+
	         '<hel:ListOfHelPdaMaintainingPlanMeasureItemAttachmentIo>'+
	            '<hel:HelMeasureItemAttachment Operation="?">'+
	               '<hel:MeasureItemParId>1-S7OU6L</hel:MeasureItemParId>'+
		'<hel:MeasureItemAttachFileName>AA</hel:MeasureItemAttachFileName>'+
		'<hel:MeasureItemAttachFileExt>jpeg</hel:MeasureItemAttachFileExt>'+
		'<hel:Id></hel:Id>'+
	               
	               '<hel:MeasureItemAttachFileBuffer>data:image/jpg;base64,</hel:MeasureItemAttachFileBuffer>'+
	            '</hel:HelMeasureItemAttachment>'+
	         '</hel:ListOfHelPdaMaintainingPlanMeasureItemAttachmentIo>'+
	      '</meas:AttachmentSynchronize_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';*/
	
	/*var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELBZMeasure" xmlns:hel1="http://www.siebel.com/xml/HEL%20Measure%20LiftElevator%20BZ">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:BZMeasureSynchronize_Input>'+
	         '<hel1:ListOfHelMeasureLiftelevatorBz>'+
	            '<hel1:HelMaintainingPlanActionTask>'+
	               '<hel1:Id>1-RWM9FQ</hel1:Id>'+
	               '<hel1:TaskStatus>未完成</hel1:TaskStatus>'+
	               '<hel1:ListOfHelMeasureItemList>'+
	                  '<hel1:HelMeasureItemList>'+
	                     '<hel1:Id>1-S7OU6L</hel1:Id>'+
	                     '<hel1:ListOfHelMeasureItemAttachment>'+
	                        
	                        '<hel1:HelMeasureItemAttachment>'+
	'<hel1:MeasureItemAttachFileName>灯塔2</hel1:MeasureItemAttachFileName>'+
	'<hel1:MeasureItemAttachFileExt>jpeg</hel1:MeasureItemAttachFileExt>'+
	'<hel1:Id>?</hel1:Id>'+
	'<hel1:MeasureItemParId>1-S7OU6L</hel1:MeasureItemParId>'+
	'<hel:MeasureItemAttachFileBuffer></hel:MeasureItemAttachFileBuffer>'+
	'</hel1:HelMeasureItemAttachment>'+
	
	                     '</hel1:ListOfHelMeasureItemAttachment>'+
	                     
	                     '<hel1:ListOfHelMeasureRecordList>'+
	                        '<hel1:HelMeasureRecordList>'+
	                           '<hel1:MeasureRecordValue>0314Test</hel1:MeasureRecordValue>'+
	                           '<hel1:Id>1-S7OU6V</hel1:Id>'+
	                        '</hel1:HelMeasureRecordList>'+
	                     '</hel1:ListOfHelMeasureRecordList>'+
	                     
	                  '</hel1:HelMeasureItemList>'+
	               '</hel1:ListOfHelMeasureItemList>'+
	               '<hel1:ListOfHelMeasureLegacy>'+
	                  '<hel1:HelMeasureLegacy>'+
	                     '<hel1:Id>1-S5JGB5</hel1:Id>'+
	                     '<hel1:MeasureLegacyOverproofReason>TEST0314</hel1:MeasureLegacyOverproofReason>'+
	                  '</hel1:HelMeasureLegacy>'+
	               '</hel1:ListOfHelMeasureLegacy>'+
	            '</hel1:HelMaintainingPlanActionTask>'+
	         '</hel1:ListOfHelMeasureLiftelevatorBz>'+
	      '</hel:BZMeasureSynchronize_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';*/

	
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELBZMeasure" xmlns:hel1="http://www.siebel.com/xml/HEL%20Measure%20LiftElevator%20BZ">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:BZMeasureSynchronize_Input>'+
	         '<hel1:ListOfHelMeasureLiftelevatorBz>'+
	            '<hel1:HelMaintainingPlanActionTask>'+
	            
	               '<hel1:Id>'+param.Id+'</hel1:Id>';  //必填
					
				   //作业项目
				   if(param.ZYXM.length>0){
					   
					   request+='<hel1:ListOfHelMeasureItemList>';  //不跟新和创建作业项目，不能有这个，
					   
						   for(var i=0;i<param.ZYXM.length;i++){
							   
							   request+='<hel1:HelMeasureItemList>';//循环 1
							   
							   		if(param.ZYXM[i].MeasureItemPostWork2){
							   			request+='<hel1:MeasureItemPostWork2>'+param.ZYXM[i].MeasureItemPostWork2+'</hel1:MeasureItemPostWork2>';  //选填
							   		}
							   		if(param.ZYXM[i].MeasureItemPreWork2){
							   			request+='<hel1:MeasureItemPreWork2>'+param.ZYXM[i].MeasureItemPreWork2+'</hel1:MeasureItemPreWork2>';   //选填
							   		}
							   		request+='<hel1:Id>'+param.ZYXM[i].Id+'</hel1:Id>'; //必填
								   
								    //附件
							   		if(param.ZYXM[i].FJ.length){
							   			request+='<hel1:ListOfHelMeasureItemAttachment>';
							   			
							   			for(var f=0;f<param.ZYXM[i].FJ.length;f++){
							   				request+='<hel1:HelMeasureItemAttachment>';  //循环2
							   					if(param.ZYXM[i].FJ[f].Id){
							   						request+='<hel1:Id>'+param.ZYXM[i].FJ[f].Id+'</hel1:Id>';//如果是新建那么ID要为？
							   					}else{
							   						request+='<hel1:Id>?</hel1:Id>';
							   					}
								   				request+='<hel1:MeasureItemParId>'+param.ZYXM[i].FJ[f].MeasureItemParId+'</hel1:MeasureItemParId>';  //必填
								   				request+='<hel1:MeasureItemAttachFileExt>jpeg</hel1:MeasureItemAttachFileExt>'; //附件类型
								   				request+='<hel1:MeasureItemAttachFileName>'+param.ZYXM[i].FJ[f].MeasureItemAttachFileName+'</hel1:MeasureItemAttachFileName>';//附件名
								   				request+='<hel1:MeasureItemAttachFileBuffer>'+param.ZYXM[i].FJ[f].MeasureItemAttachFileBuffer2+'</hel1:MeasureItemAttachFileBuffer>';//base64图片
								   				
							   				request+='</hel1:HelMeasureItemAttachment>';  //循环2
							   				
							   			}
							   			
							   			request+='</hel1:ListOfHelMeasureItemAttachment>';
							   		};
								   
								   //录入值   
								   if(param.ZYXM[i].LRZ.length>0){
									   request+='<hel1:ListOfHelMeasureRecordList>';
									   for(var l=0;l<param.ZYXM[i].LRZ.length;l++){
										   request+='<hel1:HelMeasureRecordList>';//循环3
										   		if(param.ZYXM[i].LRZ[l].MeasureRecordValue){
										   			request+='<hel1:MeasureRecordValue>'+param.ZYXM[i].LRZ[l].MeasureRecordValue+'</hel1:MeasureRecordValue>';//选填
										   		}else{
										   			request+='<hel1:MeasureRecordValue></hel1:MeasureRecordValue>';//选填
										   		}
										   		request+='<hel1:Id>'+param.ZYXM[i].LRZ[l].Id+'</hel1:Id>';//必填   暂时没有新建,如果是新建那么ID要为？
										   request+='</hel1:HelMeasureRecordList>';//循环3
									   }
									   request+='</hel1:ListOfHelMeasureRecordList>';
								   }
							   
							   request+='</hel1:HelMeasureItemList>';  //循环 1
						   }
					   
					   request+='</hel1:ListOfHelMeasureItemList>';
				   }

	               //遗留信息   已完成
	               if(param.LSYLWT.length>0){//不更新和创建遗留问题，不能有该节点，否则会删除所有遗留问题
	            	   request+='<hel1:ListOfHelMeasureLegacy>';
	            	   for(var y=0;y<param.LSYLWT.length;y++){
	            		   request+='<hel1:HelMeasureLegacy>';
			            	  	 if(param.LSYLWT[y].Id){
			            	  		request+='<hel1:Id>'+param.LSYLWT[y].Id+'</hel1:Id>';
			            	  	 }else{
			            	  		 request+='<hel1:Id>?</hel1:Id>';//必填  如果是新建那么ID要为？
			            	  	 }
			            	  	
			            	  	 request+='<hel1:MeasureLegacyProjectNo>'+param.LSYLWT[y].MeasureLegacyProjectNo+'</hel1:MeasureLegacyProjectNo>';
			            	  	 
			            	  	 if(param.LSYLWT[y].MeasureLegacySpotSituation){
			            	  		 request+='<hel1:MeasureLegacySpotSituation>'+param.LSYLWT[y].MeasureLegacySpotSituation+'</hel1:MeasureLegacySpotSituation>';  //选填
			            	  	 }else{
			            	  		request+='<hel1:MeasureLegacySpotSituation>?</hel1:MeasureLegacySpotSituation>';  //选填
			            	  	 }
			            	  	 
			            	  	 request+='<hel1:MeasureLegacyOverproofReason>'+param.LSYLWT[y].MeasureLegacyOverproofReason+'</hel1:MeasureLegacyOverproofReason>';
			            	  	 
		            	  request+='</hel1:HelMeasureLegacy>';
	                  }
	            	   request+='</hel1:ListOfHelMeasureLegacy>';
	               }
	               
	               
	               request+='</hel1:HelMaintainingPlanActionTask>'+
	         '</hel1:ListOfHelMeasureLiftelevatorBz>'+
	      '</hel:BZMeasureSynchronize_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("插入或更新保障表:"+request.toString());
	//return;
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELBZMeasure:BZMeasureSynchronize"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//提交保障表
function getBZMeasureSynchronizeTJ(Id){
	request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELBZMeasure">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:BZMeasureSubmit_Input>'+
	         '<hel:MeasureId>'+Id+'</hel:MeasureId>'+
	      '</hel:BZMeasureSubmit_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("提交保障表:"+request.toString());
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELBZMeasure:BZMeasureSubmit"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//附件删除
function getHELMeasureItemAttachmentDelete(ID){
	request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELMeasureItemAttachment">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:HELMeasureItemAttachmentDelete_Input>'+
	         '<hel:PrimaryRowId>'+ID+'</hel:PrimaryRowId>'+
	      '</hel:HELMeasureItemAttachmentDelete_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("附件删除:"+request.toString());
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMP/HELMeasureItemAttachment:HELMeasureItemAttachmentDelete"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//遗留问题删除
function getHELMeasureLegacyDelete(ID){
	request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELMeasureLegacy">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:HELMeasureLegacyDelete_Input>'+
	         '<hel:PrimaryRowId>'+ID+'</hel:PrimaryRowId>'+
	      '</hel:HELMeasureLegacyDelete_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("遗留问题删除:"+request.toString());
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMP/HELMeasureLegacy:HELMeasureLegacyDelete"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}


//阶梯表
//按条件查询阶梯表   
function getJTMeasureQuery(param){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELJTMeasure">'+
		   '<soapenv:Header/>'+
		   '<soapenv:Body>'+
		      '<hel:JTMeasureQuery_Input>'+
		         '<hel:SearchSpec>'+param.SearchSpec+'</hel:SearchSpec>'+
		         '<hel:ViewMode>'+param.ViewMode+'</hel:ViewMode>'+
		      '</hel:JTMeasureQuery_Input>'+
		   '</soapenv:Body>'+
		'</soapenv:Envelope>';
	
	WL.Logger.error("按条件查询阶梯表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELJTMeasure:JTMeasureQuery"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}


//按Id查询接梯表  ---- 暂时不使用
//功能和"按条件查询阶梯表"是一样的 条件来源于上面方法获取的数据 Id 字段
function getJTMeasureQueryById(ID){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELJTMeasure">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:JTMeasureQueryById_Input>'+
	         '<hel:PrimaryRowId>'+ID+'</hel:PrimaryRowId>'+
	      '</hel:JTMeasureQueryById_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("按Id查询接梯表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELJTMeasure:JTMeasureQueryById"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//插入或更新接梯表   
function getJTMeasureSynchronize(param){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELJTMeasure" xmlns:hel1="http://www.siebel.com/xml/HEL%20EAI%20PDA%20Measure">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:JTMeasureSynchronize_Input>'+
	         '<hel1:ListOfHelEaiPdaMeasure>'+
	            '<hel1:HelMeasureList>'+
	               '<hel1:Id>'+param.Id+'</hel1:Id>';
	               
	               //其他不良项目(保存已存在的数据必须要ID,否则已存在的数据会被删除)   OK
	               if(param.QTBLXM.length>0){
	            	   request+='<hel1:ListOfHelMeasureBaditemList>';
	            	   for(var i=0;i<param.QTBLXM.length;i++){
	            		   request+='<hel1:HelMeasureBaditemList>';
		            		   if(param.QTBLXM[i].Id){
		            			   request+='<hel1:Id>'+param.QTBLXM[i].Id+'</hel1:Id>';	            			   
		            		   }else{
		            			   request+='<hel1:Id>?</hel1:Id>';
		            		   }
		            		   
		            		   if(param.QTBLXM[i].MeasureBadItemDesc){
		            			   request+='<hel1:MeasureBadItemDesc>'+param.QTBLXM[i].MeasureBadItemDesc+'</hel1:MeasureBadItemDesc>';
		            		   }
		            		   
		            		   if(param.QTBLXM[i].MeasureBadItemResponseDivision){
		            			   request+='<hel1:MeasureBadItemResponseDivision>'+param.QTBLXM[i].MeasureBadItemResponseDivision+'</hel1:MeasureBadItemResponseDivision>';
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemZGCompleteDatetime){
		            			   request+='<hel1:MeasureBadItemZGCompleteDatetime>'+param.QTBLXM[i].MeasureBadItemZGCompleteDatetime+'</hel1:MeasureBadItemZGCompleteDatetime>';
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemWorkTime){
		            			   request+='<hel1:MeasureBadItemWorkTime>'+param.QTBLXM[i].MeasureBadItemWorkTime+'</hel1:MeasureBadItemWorkTime>';
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemZGDivision){
		            			   request+='<hel1:MeasureBadItemZGDivision>'+param.QTBLXM[i].MeasureBadItemZGDivision+'</hel1:MeasureBadItemZGDivision>';
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemResponsePersonId){
		            			   request+='<hel1:MeasureBadItemResponsePersonId>'+param.QTBLXM[i].MeasureBadItemResponsePersonId+'</hel1:MeasureBadItemResponsePersonId>';		            			   
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemReCheckDatetime){
		            			   request+='<hel1:MeasureBadItemReCheckDatetime>'+param.QTBLXM[i].MeasureBadItemReCheckDatetime+'</hel1:MeasureBadItemReCheckDatetime>';
		            		   }
		            		   if(param.QTBLXM[i].MeasureBadItemComments){
		            			   request+='<hel1:MeasureBadItemComments>'+param.QTBLXM[i].MeasureBadItemComments+'</hel1:MeasureBadItemComments>';		            			   
		            		   }
	            		   request+='</hel1:HelMeasureBaditemList>';
	            	   }
	            	   request+='</hel1:ListOfHelMeasureBaditemList>';
	               }
	               
	               
	               //作业项目
			       if(param.ZYXM.length>0){
			    	   request+='<hel1:ListOfHelMeasureItemList>';
			    	   for(var z=0;z<param.ZYXM.length;z++){
			    		   request+='<hel1:HelMeasureItemList>';
			    		   		request+='<hel1:Id>'+param.ZYXM[z].Id+'</hel1:Id>';
			    		   		
			    		   		if(param.ZYXM[z].MeasureItemNo){
			    		   			request+='<hel1:MeasureItemNo>'+param.ZYXM[z].MeasureItemNo+'</hel1:MeasureItemNo>'; //项目号			    		   			
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemName){
			    		   			request+='<hel1:MeasureItemName>'+param.ZYXM[z].MeasureItemName+'</hel1:MeasureItemName>'; //项目名称
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemType){
			    		   			request+='<hel1:MeasureItemType>'+param.ZYXM[z].MeasureItemType+'</hel1:MeasureItemType>';  //项目类型
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemContent){
			    		   			request+='<hel1:MeasureItemContent>'+param.ZYXM[z].MeasureItemContent+'</hel1:MeasureItemContent>'; //作业内容  			    		   			
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemStandard){
			    		   			request+='<hel1:MeasureItemStandard>'+param.ZYXM[z].MeasureItemStandard+'</hel1:MeasureItemStandard>'; //作业标准
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemIFGood){
			    		   			request+='<hel1:MeasureItemIFGood>'+param.ZYXM[z].MeasureItemIFGood+'</hel1:MeasureItemIFGood>'; //是否不良项
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemResponseDivision){
			    		   			request+='<hel1:MeasureItemResponseDivision>'+param.ZYXM[z].MeasureItemResponseDivision+'</hel1:MeasureItemResponseDivision>'; //责任部门
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemPreWork2){
			    		   			request+='<hel1:MeasureItemPreWork2>'+param.ZYXM[z].MeasureItemPreWork2+'</hel1:MeasureItemPreWork2>';//判断
			    		   			//这个跟着一起改变，不知道为什么  <hel1:MeasureItemJudge>?</hel1:MeasureItemJudge>
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemZGCompleteDate){
			    		   			request+='<hel1:MeasureItemZGCompleteDate>'+param.ZYXM[z].MeasureItemZGCompleteDate+'</hel1:MeasureItemZGCompleteDate>';//整改完成日期
			    		   			
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemZGWorkTime){
			    		   			request+='<hel1:MeasureItemZGWorkTime>'+param.ZYXM[z].MeasureItemZGWorkTime+'</hel1:MeasureItemZGWorkTime>';//整改人工时
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemZGDivision){
			    		   			request+='<hel1:MeasureItemZGDivision>'+param.ZYXM[z].MeasureItemZGDivision+'</hel1:MeasureItemZGDivision>';//整改部门
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemZGResponsePersonId){
			    		   			request+='<hel1:MeasureItemZGResponsePersonId>'+param.ZYXM[z].MeasureItemZGResponsePersonId+'</hel1:MeasureItemZGResponsePersonId>';//整改责任人ID
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemReCheckTime){
			    		   			request+='<hel1:MeasureItemReCheckTime>'+param.ZYXM[z].MeasureItemReCheckTime+'</hel1:MeasureItemReCheckTime>';//复检日期
			    		   			
			    		   		}
			    		   		if(param.ZYXM[z].MeasureItemReCheckPersonId){
			    		   			request+='<hel1:MeasureItemReCheckPersonId>'+param.ZYXM[z].MeasureItemReCheckPersonId+'</hel1:MeasureItemReCheckPersonId>';//复检人ID
			    		   		}
			    		   		
			    		   		//录入值    OK
			    		   		if(param.ZYXM[z].LRZ.length>0){
			    		   			request+='<hel1:ListOfHelMeasureRecordList>';
			    		   			for(var l=0;l<param.ZYXM[z].LRZ.length;l++){
			    		   				request+='<hel1:HelMeasureRecordList>';
			    		   					request+='<hel1:Id>'+param.ZYXM[z].LRZ[l].Id+'</hel1:Id>';
			    		   					
			    		   					request+='<hel1:MeasureRecordValue>'+param.ZYXM[z].LRZ[l].MeasureRecordValue+'</hel1:MeasureRecordValue>';
			    		   				
			    		   				request+='</hel1:HelMeasureRecordList>';
			    		   			}
			    		   			request+='</hel1:ListOfHelMeasureRecordList>';
			    		   		}
			    		   		
			    		   		
			    		   	//作业项目	
			    		   	request+='</hel1:HelMeasureItemList>';
			    	   }
			    	   
			    	   request+='</hel1:ListOfHelMeasureItemList>';
			       }
	               
	               
	               request+='</hel1:HelMeasureList>'+
	         '</hel1:ListOfHelEaiPdaMeasure>'+
	      '</hel:JTMeasureSynchronize_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("插入或更新接梯表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELJTMeasure:JTMeasureSynchronize"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//阶梯表提交  
function getJTMeasureSubmit(ID){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/Service/HELJTMeasure">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:JTMeasureSubmit_Input>'+
	         '<hel:MeasureId>'+ID+'</hel:MeasureId>'+
	      '</hel:JTMeasureSubmit_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("阶梯表提交:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/Service/HELJTMeasure:JTMeasureSubmit"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//其他不良项目删除
function getHELMeasureOtherBadItemDelete(ID){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELMeasureOtherBadItem">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:HELMeasureOtherBadItemDelete_Input>'+
	         '<hel:PrimaryRowId>'+ID+'</hel:PrimaryRowId>'+
	      '</hel:HELMeasureOtherBadItemDelete_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("其他不良项目删除:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMP/HELMeasureOtherBadItem:HELMeasureOtherBadItemDelete"'},
			//path:path,
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}



//公共接口
//作业内容查询,可查询：
//1.项目号
function getZYMR(param){
	
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:proj="http://siebel.com/PDAMainPlan/ProjectNoPickList">'+
	   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<proj:ProjectNoQuery_Input>'+
	         '<proj:SearchSpec>'+param+'</proj:SearchSpec>'+
	      '</proj:ProjectNoQuery_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("作业内容查询:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMainPlan/ProjectNoPickList:ProjectNoQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//作业前
function getZyFront(Id){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sel="http://siebel.com/PDAMainPlan/SelectPreWork">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<sel:PreWorkQuery_Input>'+
	         '<sel:SearchSpec>'+Id+'</sel:SearchSpec>'+
	      '</sel:PreWorkQuery_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("作业前查询:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMainPlan/SelectPreWork:PreWorkQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//作业后
function getZyHind(Id){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sel="http://siebel.com/PDAMainPlan/SelectPostWork">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<sel:PostWorkQuery_Input>'+
	         '<sel:SearchSpec>'+Id+'</sel:SearchSpec>'+
	      '</sel:PostWorkQuery_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("作业后查询:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMainPlan/SelectPostWork:PostWorkQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//查询整改责任人、复检人   和整改责任人
function getMPlanPersonQuery(param){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:mpl="http://siebel.com/PDAMainPlan/MPlanPersonPickList">'+
		   '<soapenv:Header/>'+
		   '<soapenv:Body>'+
		      '<mpl:MPlanPersonQuery_Input>'+
		         '<mpl:SearchSpec>'+param+'</mpl:SearchSpec>'+
		      '</mpl:MPlanPersonQuery_Input>'+
		   '</soapenv:Body>'+
		   '</soapenv:Envelope>';
	
	WL.Logger.error("查询整改责任人、复检人  和整改责任人:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://siebel.com/PDAMainPlan/MPlanPersonPickList:MPlanPersonQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//查询值列表   (责任部门和整改部门查询)
function getEAILOVGetListOfValues(param){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:lov="http://www.siebel.com/xml/LOVService" xmlns:lis="http://www.siebel.com/xml/ListQuery">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<lov:EAILOVGetListOfValues_Input>'+
	         '<lis:ListsQuery>'+
	            '<lis:ListQuery>'+
	               '<lis:Active>Y</lis:Active>'+
	               /*'<lis:FilterValue>?</lis:FilterValue>'+*/
	               '<lis:LanguageCode>CHS</lis:LanguageCode>'+
	               '<lis:Type>'+param.Type+'</lis:Type>'+
	            '</lis:ListQuery>'+
	         '</lis:ListsQuery>'+
	      '</lov:EAILOVGetListOfValues_Input>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("查询值列表:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"document/http://www.siebel.com/xml/LOVService:EAILOVGetListOfValues"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//所属司和所属站
function getHELCompanyStationQuery(param){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELCompanyStation">'+
		   '<soapenv:Header/>'+
	   '<soapenv:Body>'+
	      '<hel:HELCompanyStationQuery>'+
	         '<SearchSpec>'+param+'</SearchSpec>'+
	         '<ViewMode>All</ViewMode>'+
	      '</hel:HELCompanyStationQuery>'+
	   '</soapenv:Body>'+
	'</soapenv:Envelope>';
	
	WL.Logger.error("所属司和所属站:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"rpc/http://siebel.com/PDAMP/HELCompanyStation:HELCompanyStationQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//所属片
function getHELAreaQuery(param){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELArea">'+
		   '<soapenv:Header/>'+
		   '<soapenv:Body>'+
		      '<hel:HELAreaQuery>'+
		         '<SearchSpec>'+param+'</SearchSpec>'+
		         '<ViewMode>All</ViewMode>'+
		      '</hel:HELAreaQuery>'+
		   '</soapenv:Body>'+
		'</soapenv:Envelope>';
	
	WL.Logger.error("所属片:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"rpc/http://siebel.com/PDAMP/HELArea:HELAreaQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}

//所属组
function getHELGroupQuery(param){
	var request='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:hel="http://siebel.com/PDAMP/HELGroup">'+
		   '<soapenv:Header/>'+
		   '<soapenv:Body>'+
		      '<hel:HELGroupQuery>'+
		         '<SearchSpec>'+param+'</SearchSpec>'+
		         '<ViewMode>All</ViewMode>'+
		      '</hel:HELGroupQuery>'+
		   '</soapenv:Body>'+
		'</soapenv:Envelope>';
	
	WL.Logger.error("所属组:"+request.toString());
	
	var input = {
			method:'post',
			returnedContentType:'xml',
			returnedContentEncoding : 'utf-8',
			headers: {'Accept-Encoding': 'identity','SOAPAction':'"rpc/http://siebel.com/PDAMP/HELGroup:HELGroupQuery"'},
			path:'eai_chs/start.swe?SWEExtSource=WebService&SWEExtCmd=Execute&UserName=PDA&Password=PDA',
			body:{
				content:request.toString(),
				contentType:'text/xml;charset=utf-8',
			},
	};
	
	var result = WL.Server.invokeHttp(input);
	return result;
}