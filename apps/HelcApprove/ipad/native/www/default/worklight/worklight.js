  ...
  initialize: function(transport, invocationContext) {
    ...
    if (transport !== null ){
      ...
      if  ( ( ( WL.Client.getEnvironment() === WL.Environment.IPHONE) ||
              ...
              (this.responseText === '' || (this.responseText) === 'undefined') ){
        /* Try parsing NSLocalizedDescription if there is no responseText. 
         * This happens when we have a non-standard MFP response.*/
        try{
          this.responseText = WLJSX.String.interpret(transport.NSLocalizedDescription);
        }catch(e){
        }
      }
...
window.WLAuthorizationManager = (function() {
  ...
  var INVALID_CLIENT_ERROR = 'invalid_client';
  var CLIENT_AUTHENTICATION_FAILED = 'Client authentication failed';
        ...
        function(error) {
          // authorization request failed
            if ( (isUnknownClientError(error) && !invalidClientReceived) ||
               (isInvalidClientError(error) && !invalidClientReceived)) {
  ...
  function isInvalidClientError(error) {
     /*jshint camelcase:false*/
     if ((error.responseJSON.error === INVALID_CLIENT_ERROR && error.responseJSON.error_description ===
