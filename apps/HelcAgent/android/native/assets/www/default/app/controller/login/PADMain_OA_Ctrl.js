...
/* JavaScript content from app/controller/login/PADMain_OA_Ctrl.js in folder common */
Ext.define('HelcAgent.controller.login.PADMain_OA_Ctrl', {
	extend:'HelcAgent.controller.ApplicationController_OA',
	...
	oaMobileOffice:function(obj, index, target, record, e, eOpts){
		...
		if(text == '公司通讯录'){
			obj.NextView('telephonesearch_id','HelcAgent.view.OaMobileOffice.Contacts.TelephoneSearch');
			var data=[];
			...
			//符合条件
			function jieguoOne(){
				obj.NextView('installProject_ep_id','HelcAgent.view.OaMobileOffice.ElectronicProcess.installProject');
	...
	/////////////////////////////////////////////////////////////////////////////
			...
			var getResult1=function(res){
				...
				if(!store){
					store=Ext.create("HelcAgent.store.OaMobileOffice.ElectronicProcess.startTheProcessStore");
				}
			...
			var main = Ext.getCmp('padlogin_id');
      	 	if(!main){
      		 main = Ext.create('HelcAgent.view.login.PADLogin');
      	 	}
      	 	Ext.Viewport.setActiveItem(main);
      	 	ViewArray.splice(ViewArray.length-1,1);
      	 	ViewArray = [];
			//obj.showBackView('padlogin_id','HelcAgent.view.login.PADLogin');
		};
