 ...
 * 
 */
Ext.define("HelcAgent.controller.ApplicationController", {
	...
	getStore:function(storeName,FullNAME){
		  var store=Ext.data.StforeManager.get(storeName);
	 		if (!store) { 
	...
	sssjjj:function(){
		...
		if(!DataCs){
			DataCs=Ext.create('HelcAgent.store.appworkspace.CsStore');
		};
		var aa=[];
		aa[0]={one:'12'};
		aa[1]={one:'34'};
...
//					targetHeight: value,
  ...
  NextView:function(viewId,FullName){
	    ...
	    if(viewName){
  	    ...
  	    var longTimeHandler = function(handParam){
  	    	Ext.Msg.alert(handParam.field.getLabel(),handParam.field.getValue());
  	    };
  	    var selectNeedLister = view.query('selectfield');
  	    var textNeedLister = view.query('textfield');
  	    for(var j=0;j<textNeedLister.length;j++){
  	    	this.setLongTimeTap(textNeedLister[j],1.8,longTimeHandler,{field:textNeedLister[j]});
  	    }
  ...
  //返回,上一个页面
   BackView:function(){
	  var length = ViewArray.length-1;
		...
		//alert('销毁的页面：'+activeViewId);
		if(activeView)
			activeView.destroy();
		//ViewArray.splice(ViewArray.length-1,1);
		ViewArray.splice(length,1);
					...
					if (200 == httpStatusCode) {
						...
						if(TDCnum==1){
							try{
								...
								var starErr=result.invocationResult.QueryLeadPage_Output.ErrorMsg;
