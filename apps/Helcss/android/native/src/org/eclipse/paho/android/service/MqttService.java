...
public class MqttService extends Service implements MqttTraceHandler {
  ...
  private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
	...
	void showNotification(String status) {
		traceDebug(TAG, "showNotification 1() new={" + status + "}");
		...
		CharSequence contentText = "请进入相关功能模块处理！";

		Intent notificationIntent = new Intent(this, Helcss.class);
...
//				notificationIntent, PendingIntent.FLAG_UPDATE_CURRENT);
	...
	void showNotificationWithSound(String status) {
		traceDebug(TAG, "showNotificationWithSound 1() new={" + status + "}");
	    ...
	    int iconId = getResources().getIdentifier("icon", "drawable", getPackageName());
		CharSequence tickerText = status;
		CharSequence contentTitle = "mqtt";
