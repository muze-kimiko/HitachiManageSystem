...
public class MqttService extends Service implements MqttTraceHandler {
  ...
  private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
	...
	void showNotification(String status) {
		traceDebug(TAG, "---showNotification Text=" + status);
		...
		try{
			if(status.contains("{")) {
		    	jo_msg = new JSONObject(status);
		    	Ntitle = jo_msg.optString("title");
		    	Nmessage = jo_msg.optString("message");
		    }else{
		    	Ntitle = status;
		    	Nmessage = "请进入相关功能模块处理！";
		    }
		}catch (Exception e){
			traceDebug(TAG, "---"+e.getMessage());
	    }
		
	    int iconId = getResources().getIdentifier("icon", "drawable", getPackageName());
		CharSequence tickerText = Ntitle;
		CharSequence contentTitle = Ntitle;
		CharSequence contentText = Nmessage;

		
		Intent notificationIntent = new Intent(this, HelcPDA.class);
		Bundle bundle = new Bundle();
		bundle.putString("Ncontent", status);
	...
	//show a notification string
	@SuppressWarnings("deprecation")
	void showNotificationWithSound(String status) {
		traceDebug(TAG, "---showNotificationWithSound Text=" + status);
