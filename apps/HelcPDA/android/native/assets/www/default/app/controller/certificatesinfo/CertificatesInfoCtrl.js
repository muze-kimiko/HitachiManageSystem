...
/* JavaScript content from app/controller/certificatesinfo/CertificatesInfoCtrl.js in folder common */
Ext.define('HelcPDA.controller.certificatesinfo.CertificatesInfoCtrl',{
	...
	btn_Back_P_List:function(){
		//跳转页面     
		this.showBackView("CertificatesInfo_Search","HelcPDA.view.certificatesinfo.CertificatesInfo_Search");
	},
	...
	btn_Search:function(){
		//console.log('查询持证人员');
		var v_person_name = Ext.getCmp('V_PERSON_NAME').getValue();
		var v_cert_no = Ext.getCmp('V_PERSON_NAME').getValue();
		var v_vendor_name = Ext.getCmp('V_VENDOR_NAME').getValue();
		var v_job_type_code = Ext.getCmp('V_JOB_TYPE_CODE').getValue();
		if(v_person_name == '' && v_cert_no == '' && v_vendor_name == '' && v_job_type_code == '请选择'){
			Ext.Msg.alert("请至少输入一个条件！");
			return false;
		}
		
		var obj = this;
		var contentdata = {PERSON_NAME:v_person_name,PERSON_NAME:v_cert_no,VENDOR_NAME:v_vendor_name,JOB_TYPE_CODE:v_job_type_code};
		
		var CertPersonListStore = this.getStore('CertPersonListStore','HelcPDA.store.certificatesinfo.CertPersonListStore');
		CertPersonListStore.setData(null);
		//跳转页面     
		this.NextView('CertificatesInfo_List','HelcPDA.view.certificatesinfo.CertificatesInfo_List');
		
		obj.Load_Person_List(obj,contentdata);
	},
	
	Load_Person_List:function(obj,contentdata){
		...
		var getResult=function(res){
        	...
        	CertPersonListStore = obj.getStore('CertPersonListStore','HelcPDA.model.certificatesinfo.CertPersonListStore');
        	CertPersonListStore.setData(storeData);
		};
		
		this.connectServer(getResult, 'certificatesInfoAction.do?method=toSearch_Inst_Person', JSON.stringify(contentdata));
	},
