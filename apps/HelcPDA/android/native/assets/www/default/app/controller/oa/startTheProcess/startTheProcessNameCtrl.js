...
/* JavaScript content from app/controller/oa/startTheProcess/startTheProcessNameCtrl.js in folder common */
 Ext.define('HelcPDA.controller.oa.startTheProcess.startTheProcessNameCtrl',{
	...
	qc_returnStartTheProcessName : function(){
		...
		this.showBackView('qc_StartprocessName_id','HelcPDA.view.oa.startTheProcess.startTheProcessName');
		var store=this.getStore('personnelSelectionS','HelcPDA.store.oa.startTheProcess.DailyOffice.Idea.personnelSelectionS');
		store.setData([]);
	...
	public_ToSelectNode : function(){
		...
		var store=this.getStore('approvalOpinionS','HelcPDA.store.oa.startTheProcess.DailyOffice.Idea.approvalOpinionS');
		store.setData([]);
		var getResult3=function(res){
            ...
            if(acti.node==undefined||acti.node==null||acti.node==""||acti.node=={}){
            	Ext.Msg.alert('不存在！');
            }else{
            	...
            	for(var nb=0;nb<lines.length;nb++){
            		...
            		tp_data.url=getAjaxOption(lines[nb].to,acti);//获取人员URL
            		tp_data.cfg=getNextCFG(lines[nb].to,acti);//对应节点配置信息
            		cc.log(tp_data.cfg);

            		//根据不用流程和流程所提交的数据，对流程结点的显示不同
            		if(lines[nb].conds!=""&&conds.indexOf(lines[nb].conds)!=-1){
            			userSolist.push(tp_data);
           			}else if(lines[nb].conds==""&&conds.indexOf("@")!=-1){
           				userSolist.push(tp_data);
    			...
    			var ViewId = Ext.Viewport.getActiveItem().id;
    			obj_this.NextView('qc_approvalOpinion_ID','HelcPDA.view.oa.startTheProcess.DailyOffice.Idea.approvalOpinion');
    			//存储页面ID，用于提交数据后销毁
		...
		var content ={method:'ForApprovalProcess', 'task_id':taskid , 'piid':piid};
		cc.log(content);
...
//分类：日常办公
		...
		if(bpdName == "用印申请"){
			//验证必填
			var IdArray = ['agentman','subject','fenshu','sqliyou_textarea'];
			if(this.isRequired(IdArray)){
				return;
			}
		}else if(bpdName == "出差申请"){
			//验证必填
			var IdArray = ['ygh','prefee','otherfee','feesum','phone','subject','place','starttime',
			               'rettime','reason_textarea','peonum','projectno','projectname'];
			if(this.isRequired(IdArray)){
				return;
			}
			var IdTrave = ['ygh','sendmobile','sendnumber','plant','phone_sfz','phone'];
//			验证出差申请起草是否符合格式
			...
			this.worry();
			this.getcomufee();
			this.getApplication().getController('oa.startTheProcess.DailyOffice.travelRequestCtrl').FreeControl();
			return;
		...
		//分类：营业/工程业务
	    	...
	    	var IdArray = ['apellation','address','party','phone','date','count',
	    	               'regh','phone2','subject','report_textarea'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    	this.getApplication().getController('oa.startTheProcess.BusinessService.MaintainTransformCtrl').setvaluedate();
	    }
		//分类：质量控制
	    else if(bpdName == "开箱补缺件及不良问题反馈报告"){
	    	var IdArray = ['dpm','bjm','wtm','subject','ts','yhmc','qwclqx','jjdz','yb',
	    	               'shr','phone','shr_2','phone_2','scgh','azdw','azzz','hth','beizhu_textarea',
	    	               'bgrphone'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    	if(!this.getApplication().getController('oa.startTheProcess.QualityControl.KXBQJCtrl').checkvalue18()){
	    		return;
	    	};
	    }
	    else if(bpdName == "三包申请报告"){
	    	var IdArray = ['subject','bdtype','serviceaddr','acceptor','acceptno','produceno','hth','dhdw','typeno','power','checkdate',
	    	               'sertime','parts','partsxh','errdate','errorstatus','errcontent','textarea','textarea2','textarea3'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    	if(!this.getApplication().getController('oa.startTheProcess.QualityControl.ThreeGuaranteesCtrl').checkvalue()){
	    		
	    	}
	    }
	    else if(bpdName == "质量部投诉流程"){
	    	var IdArray = ['subject','customname','tsflag','tel','contractno','tsdate','tscontect_textarea'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    }
	    	...
	    	var IdArray = ['createdate','phone2','subject','report_textarea','apellation','address','party',
	    	               'phone','date','produceno','model','floor','date1','date2','unit','count','refermodel'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    }
	    else if(bpdName=="开具发票"){
	    	var IdArray = ['zfrq','hth','gh','subject','kpje','kpbl','kpjegc','kpjeyf','htzxqksm','yyyq_textarea'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    }
	    else if(bpdName=="诉讼审批流程"){
	    	var IdArray = ['hth','xmxz','dwmc1','dwdz1','phone1','lxr1','htdep',
	    	               'httype','ts1','ts2','paymoney','reason_textarea'];
	    	if(this.isRequired(IdArray)){
	    		return;
	    	}
	    	if(!this.getApplication().getController('oa.startTheProcess.BusinessService.litigationApproveCtrl').checkvalue35()){
	    		return;
	    	}
	    }
		//分类：提案管理流程
	    	...
	    	if(!this.getApplication().getController('oa.startTheProcess.ProposalManage.PM_TAGLLC_NGCtrl').commit()){
	    		return;
	    	}
		}
		else{
	...
	init1:function(obj, index, target, record, e, eOpts){
		...
		var ibpd={'appid':record.data.appid,'bpdid':record.data.bpdid,'bpdname':record.data.bpdname,'snapshotid':record.data.snapshotid};
		var iarg={'idrafter':publicId};
		cc.log('----iarg----');
		cc.log(iarg);
		...
		//分类：日常办公
		if(bpdname=='工作联络书'){ 
			this.NextView('qc_jobContactBook_ID','HelcPDA.view.oa.startTheProcess.DailyOffice.jobContactBook');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
			Ext.getCmp('qc_bpdname').setValue(record.data.bpdname);
		}else if(bpdname=='出差申请'){
			this.NextView('travelRequest_ID','HelcPDA.view.oa.startTheProcess.DailyOffice.travelRequest');
			//给出发时间和返回时间添加监听器
			var starttime=Ext.getCmp('starttime');
				starttime.addListener('change',obj_this.starttime1,this,{
			});
			var rettime=Ext.getCmp('rettime');
				rettime.addListener('change',obj_this.rettime,this,{
			});
			var otherfee=Ext.getCmp('otherfee');
				otherfee.addListener('change',obj_this.getcomufee,this,{
			});
...
//			return;
			this.NextView('qc_useStamp_id','HelcPDA.view.oa.startTheProcess.DailyOffice.useStamp');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		//分类：营业/工程业务
		}else if(bpdname=='维修改造工程业务联络流程'){
			this.NextView('qc_MaintainTransformView_id','HelcPDA.view.oa.startTheProcess.BusinessService.MaintainTransformView');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		}else if(bpdname=='开具发票'){
//			Ext.Msg.alert("请在电脑端起草！");
//			return;
			this.NextView('qc_Invoice_id','HelcPDA.view.oa.startTheProcess.BusinessService.Invoice');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		//分类：质量控制
		}else if(bpdname=='开箱补缺件及不良问题反馈报告'){
			this.NextView('qc_KXBQJView_id','HelcPDA.view.oa.startTheProcess.QualityControl.KXBQJView');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		}else if(bpdname=='三包申请报告'){
			this.NextView('qc_ThreeGuarantees_id','HelcPDA.view.oa.startTheProcess.QualityControl.ThreeGuarantees');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		}else if(bpdname=='质量部投诉流程'){
			this.NextView('qc_QualityComplain_id','HelcPDA.view.oa.startTheProcess.QualityControl.QualityComplain');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		//分类：提案管理流程
		}
		else if(bpdname=='提案管理流程'){
			this.NextView('qc_PM_TAGLLC_NG_id','HelcPDA.view.oa.startTheProcess.ProposalManage.PM_TAGLLC_NG');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
			Ext.getCmp('no').addListener('change',obj_this.GetYGInfo,this,{});
		}

		else if(bpdname=='非标报告作业处理流程'){
			this.NextView('qc_nonstandardWork_id','HelcPDA.view.oa.startTheProcess.BusinessService.nonstandardWork');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		}
		else if(bpdname=='诉讼审批流程'){
//			Ext.Msg.alert("请在电脑端起草！");
//			return;
			this.NextView('qc_litigationApprove_id','HelcPDA.view.oa.startTheProcess.BusinessService.litigationApprove');
			Ext.getCmp('qc_surface_ID').setTitle(record.data.bpdname);
		}
...
//    				for(key in jsonObj.data.fdata.mast){
    			...
    			//
    			Ext.getCmp('agentman').setValue(OA_usernames);
    			Ext.getCmp('dept').setValue(OA_dept);
    		...
    		var content ={method:'ForApprovalProcess',task_id:data.taskid, piid:data.piid};
    		cc.log(content);
	...
	ygh1 : function(obj, newDate, oldDate, eOpts){
		...
		var getResult=function(res){
			if(res.body != "()"){
				...
				Ext.getCmp('subject').setValue(data[0].emp_name+"出差申请");
		...
		var getResult=function(res){
			if(res.body != "()"){
				...
				if (data[0].Success=="true"){
					Ext.getCmp('projectname').setValue(data[0].ProjectCode);
				}else{
...
//				Ext.getCmp('query_zw').setValue(data[0].zhiwu);
				...
				Ext.getCmp('sendreader').setValue(data[0].emp_name+" "+ygh);
		...
		var getResult=function(res){
			...
			if(res.body != "()"){
                ...
                if (country=="日本"){
                   ...
                   if (zw=="总裁" || zw=="副总裁"||zw=="总经理" ||zw=="副总经理" ||  zw=="主任" || zw=="主席" ) {
                    level="1";
                    zs=650; 
                    bt=120;
                   }
                   else if (zw=="部长" || zw=="副部长" || zw=="总监" || zw=="副总监" || zw=="总经理助理") {
                    level="2";
                    zs=500;
                    bt=100;
                    }
			    ...
			    var days=obj_this.compareDate(rdate,starttime,"-");
			    prefee=(jt + zs*(days - 1) + bt*days)*peonum;
//			    alert("交通费："+jt+"住宿：" + zs + "补贴："+bt + "天数：" + days);
			    Ext.getCmp('prefee').setValue(prefee);
			    Ext.getCmp('feesum').setValue(prefee+parseInt(Ext.getCmp('otherfee').getValue()));
				}
			};
	 ...
	 worry : function (){
		 ...
		 if (waypath=="2")
		   {
			 Ext.getCmp('tiaojian01').setValue("y1");   // "y1" //这里写路有线条件
		   }
		 if (waypath=="3")
		   {
			 Ext.getCmp('tiaojian01').setValue("y2"); // "y2" //这里写路有线条件
		   }
		 if (waypath=="4")
		   {
			 Ext.getCmp('tiaojian01').setValue("y3"); // "y3" //这里写路有线条件
		   }
...
//		 if("$!{acti.node.name}"=="起草"){
		    if((zbxm=="基建、机电工程" || zbxm=="重要设备、原材料等货物的采购")&&(Ext.getCmp('contractyear').getValue()>=500000)){
		       result="@";
		     }
		     else if((zbxm=="维修项目及施工" || zbxm=="软件及管理项目、工装夹具采购" || zbxm=="办公用品设备或废旧设备、物资项目" || zbxm=="办公用品设备" || zbxm=="废旧设备、物资项目" || zbxm=="其他") && (Ext.getCmp('contractyear').getValue()>=300000) && (Ext.getCmp('contractyear').getValue()<500000)){
		       result="@";
		     }else 
		         {        
		             result="y1";
...
//		             result="@,y3";
//		         }
//		         else if((document.all.contractyear.value>50000)&&(contracttype!="合同撤消")){
//		             result="@,y2";
//		         }
//		         else{
//		             result="@,y1";
...
//		             result="@,y2";//写条件
//		         }
//		     else if((document.all.contractyear.value>50000)&&(document.all.contractyear.value<=300000)&&(contracttype=="合同撤消")){
//		             result="@,y1";
//		         }
//		     else {
//		             result="@";    
...
//	     }
	...
	//定义完毕
	    ...
	    if (zw=="总裁" || zw=="副总裁") {        //总裁级出差
	    ...
	    else if (zw=="总经理" ||  zw=="主任" || zw=="主席" || ygbh=="00001323") {        //总经理级出差，总裁审批
	        if (zjl=="") {
	            ...
	            Ext.getCmp('firflow').setValue(zjl+" "+zjlno);
	            return false;
	        }
	    }
	    else if (zw=="部长" || zw=="副部长" || zw=="总监" || zw=="副总监" || zw=="总经理助理") {        //部长级出差
	        if (area=="省内") {        //部长级境内出差
	            if (bbz!="") {
	            	Ext.getCmp('way').setValue(bbz+" "+bbzno);
		            Ext.getCmp('waypath').setValue("1");
		            Ext.getCmp('firflow').setValue(bbz+" "+bbzno);
	                return false;
	            }
	            else{
	    ...
	    else if (zw=="科长" || zw=="经理" || zw=="部长助理" || zw=="总监助理") {        //科长级出差
	        if (area=="省内") {        //科长级境内出差
	            if (plant=="否") {        //不须乘机
	                if (bz!="") {        //存在部长级领导提交部长级审批
	                	Ext.getCmp('way').setValue(bz+" "+bzno);
			            Ext.getCmp('waypath').setValue("1");
			            Ext.getCmp('firflow').setValue(bz+" "+bzno);
	                    return false;
	                }
	                else{
	            ...
	            if (plant=="是") {        //须要乘机
	                if (bz!="" && bbz!="") {        //存在部长级领导提交部长及本部长审批
	                    tmp1=bz+" "+bzno;
	                    tmp2=bbz+" "+bbzno;
	                    Ext.getCmp('way').setValue(tmp1+"-->"+tmp2);
	        ...
	        if (area=="省内") {        //普通员工境内出差
	            if (plant=="否") {        //不须乘机
	                if ( staytime>1) {        //发生费用或非当日出差
	                    ...
	                    if (kz!="") {        //提交科长审批
	                    	Ext.getCmp('way').setValue(kz+" "+kzno);
	    		            Ext.getCmp('waypath').setValue("1");
	    		            Ext.getCmp('firflow').setValue(kz+" "+kzno);
	                        return false;
	                    }
	                    else if (kz=="" && bz!="") {        //科长为空，提交部长审批
	                    	Ext.getCmp('way').setValue(bz+" "+bzno);
	            ...
	            if (plant=="是") {        //须要乘机
	                if (kz!="" && bz!="" && bbz!="") {        //提交科长、部长、本部长审批
	                    ...
	                    tmp2=bz+" "+bzno;
	                    tmp3=bbz+" "+bbzno;
	                    Ext.getCmp('way').setValue(tmp1+"-->"+tmp2+"-->"+tmp3);
	                ...
	                else if (kz=="" && bz!="" && bbz!="") {        //科长为空，提交部长、本部长审批
	                    tmp1=bz+" "+bzno;
	                    tmp2=bbz+" "+bbzno;
	                    Ext.getCmp('way').setValue(tmp1+"-->"+tmp2);
    		            Ext.getCmp('waypath').setValue("2");
	                ...
	                else if (kz=="" && bz=="" && bbz!="") {        //科长、部长为空，提交本部长审批
	                	Ext.getCmp('way').setValue(bbz+" "+bbzno);
    		            Ext.getCmp('waypath').setValue("1");
    		            Ext.getCmp('firflow').setValue(bbz+" "+bbzno);
	                    return false;
	        ...
	        if (area=="省外") {        //普通员工境外出差
	            if (kz!="" && bz!="" && bbz!="" && zjl!="") {        //提交科长、部长、本部长、总裁审批
	                tmp1=kz+" "+kzno;
	                tmp2=bz+" "+bzno;
	                tmp3=bbz+" "+bbzno;
	                tmp4=zjl+" "+zjlno;
	            ...
	            else if (kz=="" && bz!="" && bbz!="" && zjl!="") {        //科长为空，提交部长、本部长、总裁审批
	                tmp1=bz+" "+bzno;
	                tmp2=bbz+" "+bbzno;
	            ...
	            else if (kz=="" && bz=="" && bbz!="" && zjl!="") {        //科长、部长为空，提交本部长、总裁审批
	                tmp1=bbz+" "+bbzno;
	...
	starttime1 : function(obj, newDate, oldDate, eOpts){
		var tempEndTime=Ext.getCmp('rettime').getValue()+" "+"00:00:00";
		var tempStartTime=newDate+" "+"00:00:00";
		if(newDate!=''){
	    	if(Ext.getCmp('rettime').getValue()!=''){
	    		if(Date.parse(tempStartTime)>Date.parse(tempEndTime)){
    ...
    rettime1 : function(obj, newDate, oldDate, eOpts){
		...
		var tempStartTime=Ext.getCmp('starttime').getValue()+" "+"00:00:00";
		if(newDate!=''){
  ...
  //监听-公务用车联络流程-出发时间-返回时间
	chufa : function(obj, newDate, oldDate, eOpts){
		var tempEndTime=Ext.getCmp('ycdate1').getValue()+" "+"00:00:00";
		var tempStartTime=newDate+" "+"00:00:00";
		if(newDate!=''){
    ...
    fanhui : function(obj, newDate, oldDate, eOpts){
    	var tempEndTime=newDate+" "+"00:00:00";
		var tempStartTime=Ext.getCmp('ycdate').getValue()+" "+"00:00:00";
		if(newDate!=''){
    ...
    FreeInfo : function(){
		var obj_this = this;
		var IdArray = ['peonum','peonum2','peonum3','starttime','rettime','country'];
		if(this.isRequired(IdArray)){
			return;
		}
		var gj = Ext.getCmp('country').getValue();
		gj=encodeURIComponent(gj);
		var pcode = '/unioa/processdata.nsf/getjwbt?openagent&unid='+gj+'&_dc=&callback=';
		var getResult=function(res){
			if(res.html.body!='()'){
				var data3 = eval(res.html.body);
				var data = data3.data;
				...
				var peonum=parseFloat(Ext.getCmp('peonum').getValue());    
				var peonum2=parseFloat(Ext.getCmp('peonum2').getValue());    
				var peonum3=parseFloat(Ext.getCmp('peonum3').getValue());    
				var starttime=Ext.getCmp('starttime').getValue();    
				var rdate=Ext.getCmp('rettime').getValue();    
				...
				var feesum=days*peonum*(fs1 + zs1 + bt1) + days*peonum2*(fs2 + zs2 + bt2) + days*peonum3*(fs3 + zs3 + bt3);
				Ext.getCmp('biz').setValue(bz);
				Ext.getCmp('prefee').setValue(feesum);
				Ext.getCmp('feesum').setValue(feesum + parseFloat(Ext.getCmp('otherfee').getValue()));
			}
		};
  ...
  //监听-公务用车联络流程-出发时间-返回时间
	kaishi : function(obj, newDate, oldDate, eOpts){
		var tempEndTime=Ext.getCmp('yxnyz').getValue()+" "+"00:00:00";
		var tempStartTime=newDate+" "+"00:00:00";
		if(newDate!=''){
    ...
    jieshu : function(obj, newDate, oldDate, eOpts){
    	var tempEndTime=newDate+" "+"00:00:00";
		var tempStartTime=Ext.getCmp('yxnyq').getValue()+" "+"00:00:00";
		if(newDate!=''){
...
//			}
    ...
    GSFW_checkvalue34:function(gsfw_data){
    	...
    	var frm_arcpath_value=Ext.getCmp('arcpath').getValue();
    	
    	var frm_jcacl_value=Ext.getCmp('jcacl').getValue();
    	var frm_catalogname_value=Ext.getCmp('catalogname').getValue();
    	//var frm_pbsj_value=Ext.getCmp('pbsj').getValue();
    	//alert(gsfw_data);

    	if(gsfw_data=="拟稿") {
    		//alert(frm_dept_value);
    		if(frm_dept_value=="秘书科" || frm_dept_value=="综合管理科" || frm_dept_value=="公关宣传科" || frm_dept_value=="总裁办" || frm_dept_value=="总裁办公室"){
    			//frm.iszcb.value="1";
    	...
    	if (gsfw_data=="排版A"||gsfw_data=="排版B") {
    	    ...
    	    Ext.getCmp('bzh').setValue(username);
    	        ...
    	        || frm_miji_value=="" || frm_showss_value=="" || frm_bcnx_value=="" 
    	        || frm_bzr_value=="" || frm_bzsj_value=="" || frm_shr_value=="" 
    	        || frm_shsj_value=="" || frm_pzr_value=="" || frm_pzsj_value=="" 
    	        || frm_zwdoc_value=="" || frm_arcpath_value==""){
    	            alert('请输入相关信息，再提交!');
    	        ...
    	        /*if (frm_jcacl_value=="[object]"){
    	            if (frm.jcacl.value==""){
    	              alert('请选择继承权限，再提交!');
    	              frm.jcacl.style.background='#e8e8e8';
    	               return;
    	             };
    	             if (frm.jcacl.value=="不继承" && frm.readpeo.value==""){
    	                alert('请选择阅读人员!');
    	        ...
    	        /*var fwtype;
    	        for (i=0;i<frm.fwtype.length;i++) {
    	            if (frm.fwtype[i].checked==true) {
    	                fwtype=frm.fwtype[i].value;
    	            }
    	        }*/
    	        /*if (frm_fwtype_value=="公司发文" && frm.writ.value==""){
    	            alert('请输入相关信息，再提交!');
    	            frm.writ.style.background='#e8e8e8';
    	            frm.writ.focus();
    	            return;
    	        }*/
    	        if (frm_fwtype_value=="规章制度" && frm_catalogname_value==""){
    	            alert('请输入相关信息，再提交!');
    ...
    GSWF_selzw:function(fieldn){
    	var url=_oa_path+"/unioa/processtrace.nsf/selectatt?openform&unid="+document.all.piid.value; 
    	var frm=document.all;
    	var result=getURLObj(url);
    	var wz=result.indexOf("*");
    	frm.zwdoc.value=result.substring(0,wz);
    	frm.zwdocunid.value=result.substring(wz+1,result.length);
    },
    
    //选择文档归档位置 的一个选择按钮
    GSWF_selectcatalogId:function(){
    	var frm=document.all;
    	var url=_oa_path+"/oa/workflowgdpz.nsf/selectcatalog?OpenForm&selflag="+frm.noselect.value+"&sjk="+frm.cabinet.value+"&gdsort="+frm.pigeonhole.value
    ...
    monenyChange:function(){
    	...
    	Ext.getCmp('feesum2').setValue(parseFloat(hj1)+parseFloat(hj2)+parseFloat(hj3)+parseFloat(hj4)+parseFloat(hj5));
    },
    test22:function(){
    	var getResult=function(res){
//			if(res.body != "()"){
