...
/* JavaScript content from app/controller/androidMap/AndroidMapTbCtrl.js in folder common */
/**
 * 地图模块的事件
 */
var pageName;
Ext.define('HelcPDA.controller.androidMap.AndroidMapTbCtrl',{
	extend:'HelcPDA.controller.ApplicationController',
	config:{
		control:{
			//周围的人的操作
			"button#showLocationTB":{
				tap:'showLocation'
			},
			"button#arroudToMapTB":{
				tap:'arroudToMap'
			},
			//我的信息的操作
			"list#myDetaillListTB":{
			...
			"list#EmpListTB":{
				itemtap:'EmpList'
			},"button#EmpBackViewTB":{
				tap:'EmpBackView'
			},"button#serachBleowRoultTB":{
				tap:'serachBleowRoult'
			},"button#serachBelowNowTB":{
				tap:'serachBelowNow'
			},"list#belowRoultTB":{
				itemtap:'belowRoult'
			},"button#allRoultTB":{
				tap:'allRoult'
			},"button#belowManDetailBackTB":{
				tap:'belowManDetailBack'
			},"button#searchCardNumTB":{
				tap:'searchCardNum'
			},"list#sendCard_listTB":{
				itemtap:'sendCard_list'
			},"button#sendCardBackTB":{
				tap:'sendCardBack'
			}
		},
	},
	...
	showLocation:function(){
	   ...
	   function getResult(res){
		   ...
		   if(length>0){
			   obz.NextView('whitePage','HelcPDA.view.androidTBMap.WhitePage');
...
//						obj.NextView('nearView','HelcPDA.view.androidMap.NearView');
						...
						obz.BackView();
						obz.BackView();
						obz.NextView('selfMyView','HelcPDA.view.androidTBMap.MyView');
						...
						obz.BackView();
						obz.BackView();
						obz.NextView('sendCard','HelcPDA.view.androidTBMap.SendCard');
					...
					},"JSTBMapMain","全部周围的人",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase(),ArroudPeople:res.item}]);   
		   }else{
	...
	arroudToMap:function(){
		 ...
		 cordova.exec(function(res) {
				...
				if("正常返回"==res[0].LocationFlag){
					...
					WL.Toast.show("错误:"+err);
				},"JSTBMapMain","进入地图",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase()}]);   
	},
	//我的信息
		...
		if(index==0){
			...
			function getResult1(res){
				...
				if(res.item.length>0){
					...
					jsData[1]=res.item;
					obz.NextView('empView','HelcPDA.view.androidTBMap.EmpView');
...
//					var EmpList=Ext.getCmp('EmpList');
			...
			// obz.NextView('whitePage','HelcPDA.view.androidMap.WhitePage');
			 cordova.exec(function(res) {
					if("正常返回"==res[0].LocationFlag){
						...
						WL.Toast.show("错误:"+err);
					},"JSTBMapMain","进入离线",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase()}]); 
	...
	checkRoult:function(){
		var obz=this;
		mineDate=Ext.getCmp('mineDateTB').getValue();
		//alert(mineDate);
		var obj=jsData[0];
...
//			   cordova.exec(function(res) {
					//jsData=res;
			       pageName="我的轨迹";
				   obz.NextView('belowList','HelcPDA.view.androidTBMap.BelowList');
	...
	mineToMap:function(){
		 ...
		 cordova.exec(function(res) {
				...
				if("正常返回"==res[0].LocationFlag){
					...
					obz.BackView();
					obz.BackView();
					obz.NextView('nearView','HelcPDA.view.androidTBMap.NearView');
				}
				else if("我的信息"==res[0].LocationFlag){
					obz.BackView();
					//obz.NextView('selfMyView','HelcPDA.view.androidMap.MyView');
...
//					},"JSMapMain","我的轨迹",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase(),MineRoult:res.item}]);   
    ...
    belowRoult:function(obk,index,target,record,e,eOpts ){
      ...
      cordova.exec(function(res) {
		...
		if("正常返回"==res[0].LocationFlag){
			...
			WL.Toast.show("错误:"+err);
		},"JSTBMapMain","我的轨迹",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase(),MineRoult:tempRes}]); 
    },
    //全部下属轨迹
    allRoult:function(){
    	var obz=this;
    ...
    cordova.exec(function(res) {
		if("正常返回"==res[0].LocationFlag){
			...
			WL.Toast.show("错误:"+err);
		},"JSTBMapMain","我的轨迹",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase(),MineRoult:res.item}]);   
    ...
    function getResult1(res){
		...
		if(length<1){
			...
			var sendCard_list=Ext.getCmp('sendCard_listTB');
    ...
    sendCard_list:function(obk,index,target,record,e,eOpts ){
        ...
        obz.MP_ID=MP_ID;
        obl[0]=obz;
		obj.NextView('whitePage','HelcPDA.view.androidTBMap.WhitePage');
		cordova.exec(function(res) {
			jsData=res;
			if("正常返回"==res[0].LocationFlag){
				obj.BackView();
				...
				obj.BackView();
				obj.BackView();
				obj.NextView('nearView','HelcPDA.view.androidTBMap.NearView');
			}else if("我的信息"==res[0].LocationFlag){
				obj.BackView();
				obj.BackView();
				obj.NextView('selfMyView','HelcPDA.view.androidTBMap.MyView');
			}else if("返回主页"==res[0].LocationFlag){
				obj.BackView();
				obj.BackView();
			}else if("工号打卡"==res[0].LocationFlag){
				...
				WL.Toast.show("错误:"+err);
			},"JSTBMapMain","工号进入",[{USERID:userid,UUID:device.uuid,meter:500,isLeader:isLeader,DeviceNo:(device.uuid).toUpperCase(),obz:obl}]); 
