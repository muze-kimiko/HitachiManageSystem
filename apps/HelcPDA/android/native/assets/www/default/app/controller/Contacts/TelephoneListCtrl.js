...
/* JavaScript content from app/controller/Contacts/TelephoneListCtrl.js in folder common */
Ext.define('HelcPDA.controller.Contacts.TelephoneListCtrl',{
	...
	telephonelist_id_FH:function(){
		this.showBackView('telephonesearch_id','HelcPDA.view.Contacts.TelephoneSearch');
		//清空
	...
	telephonelist_id_list:function(dataview, index, target, record, e, eOpts){
		...
		if(event.target.id!='telid'){
			this.NextView('telephonedetailed_id','HelcPDA.view.Contacts.TelephoneDetailed');
			var datads=Ext.data.StoreManager.get('TelephoneAddressStore');
			if(!datads){
				datads=Ext.create('HelcPDA.store.Contacts.TelephoneAddressStore');
			};
