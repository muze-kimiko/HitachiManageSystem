...
/* JavaScript content from app/controller/OpportunityManagement/Project_New/BusinessAgentCtrl.js in folder common */
Ext.define('HelcPAD.controller.OpportunityManagement.Project_New.BusinessAgentCtrl',{
	...
	businessagent_new_id_BC:function(){
		var AN=Ext.getCmp('businessagent_new_id_BC').getText();
		var TS='保存代理商业绩？';
		if(AN=='修改'){
			TS='修改代理商业绩？';
		};
		...
		Ext.Msg.show({
			...
			fn:function(buttonId){
				if(buttonId=='yes'){
					...
					var primaryField = Ext.getCmp('agentAchievePrimaryField').getValue();
							...
							Ext.Msg.show({
								...
								fn:function(buttonId){
									if(buttonId=='yes'){
										obj.BackView();
										obj.BackView();
										obj.getApplication().getController('HelcPAD.controller.OpportunityManagement.Project_New.ProjectInfoCtrl').projectinfo_new_id_QTXX_DLS();
									}
								}
							});
	...
	businessagent_new_id_FH:function(){
		if(!this.record)
			...
			this.getStore('BusinessAgentStore','HelcPAD.store.OpportunityManagement.Project_New.BusinessAgentStore').addData(this.record);
			this.BackView();
		}
	...
	businessagentlist_new_id_ZJ:function(){
		var opportunity = Ext.getCmp('businessAgentOpportunity').getData(); 
		this.NextView('businessagent_new_id','HelcPAD.view.OpportunityManagement.Project_New.BusinessAgent');
	...
	submitAgentAchieve:function(){
		...
		var agentStore = this.getStore('BusinessAgentStore','HelcPAD.store.OpportunityManagement.Project_New.BusinessAgentStore');
	...
	businessagentlist_new_id_list:function(dataview, index, target, record, e, eOpts){
		if(record.get('LeadStatus')=='display:none;')
			return ;
		if(event.target.id!='conkung_businessagentlist'){
			...
			this.NextView('businessagent_new_id','HelcPAD.view.OpportunityManagement.Project_New.BusinessAgent');
			//按钮变为修改
			Ext.getCmp('businessagent_new_id_BC').setText('修改');
			this.toInit();
			Ext.getCmp('businessAgentOppty').setData(businessAgentOpportunity);
			Ext.getCmp('businessagent_new_id').setRecord(record);
			//当商机为完成的时候才能修改
			if(businessAgentOpportunity.OpptyStatus=='完成'&&(businessAgentOpportunity.AgentPerformanceStatus=='新建'||businessAgentOpportunity.AgentPerformanceStatus=='修改中')&&object.getController('OpportunityManagement.Project_New.BeInChargeOf.ProjectDirectorMainCtrl').operationProfile==null){
				//输入比例
				Ext.getCmp('achievePrecent').setReadOnly(false);
				//说明
				Ext.getCmp('comments').setReadOnly(true);
			}
			if(businessAgentOpportunity.OpptyStatus=='完成'&&(businessAgentOpportunity.AgentPerformanceStatus=='新建'||businessAgentOpportunity.AgentPerformanceStatus=='修改中'||businessAgentOpportunity.AgentPerformanceStatus=='审批中')&&object.getController('OpportunityManagement.Project_New.BeInChargeOf.ProjectDirectorMainCtrl').operationProfile){
				Ext.getCmp('achievePrecent').setReadOnly(true);
				Ext.getCmp('comments').setReadOnly(false);
				Ext.getCmp('agentAchievePrimaryField').setReadOnly(true);
			}
			if(object.getController('OpportunityManagement.Project_New.BeInChargeOf.ProjectDirectorMainCtrl').operationProfile){
				Ext.getCmp('agentAchievePrimaryField').setReadOnly(true);
				Ext.getCmp('achievePrecent').setReadOnly(true);
				Ext.getCmp('comments').setReadOnly(true);
			}
				
			Ext.getCmp('businessAgentName').setReadOnly(true);
			
			var agentAchievePrimaryField = Ext.getCmp('agentAchievePrimaryField');
			if(record.data.PrimaryField=='Y')
	...
	toInit:function(){
		...
		var getResult = function(result){
			...
			if(obj.record)
				select.setValue(obj.record.data.Agent);
		};
