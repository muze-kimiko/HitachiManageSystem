...
Ext.define('HelcPAD.controller.OpportunityManagement.CustomerInformation_New.CustomListCtrl',{
	...
	customlist_new_id_XJKH:function(){
		...
		var Number=DataCs.getCount();
		/*console.log(DataCs.data);
		console.log(DataCs.data.items[0].data.EBSname);
		console.log(DataCs.data.items[0]);
		console.log(Number);*/
		...
		/*if(Number>0){
			var orange='';
			try{
				orange=DataCs.data.items[0].data.ListOfAccount_Organization.Account_Organization.Organization;
			}catch(e){
			}
			console.log(orange);
			if(DataCs.data.items[0].data.EBSname!=1&&orange=='营业工程总部'){
		...
		for(var i=0;i<DatType.length;i++){
			obj.publicZLB(DatType[i],DataKJ[i],false);
		};
		
		/**
		 * 新建客户时部分下拉列表默认值 xcx 17-6-26
		 */
		//客户性质
		Ext.getCmp('S_AccountProperty').setValue('普通社会组织');
		//保养客户标识
		Ext.getCmp('S_AccountMPType').setValue('普通');
		//客户分类
		Ext.getCmp('S_AccountSort').setValue('国内合同客户');
		//是否特批
		...
		//list滑动删除
		this.getAddSliedDelete('custominfo_new_id_listV','OpportunityManagement.CustomerInformation_New.CustomInfoCtrl');
	...
	customlist_new_id_list:function(dataview, index, target, record, e, eOpts){
		...
		getResult=function(data){
			...
			if(Organization=='营业工程总部'){
				...
				}else if(record.data.AccountStatus=='有效'){
				...
				if(record.data.AccountStatus=='潜在'){
					//判断是否有总公司
					var Datazgs=object.getController('OpportunityManagement.CustomerInformation_New.CustomSearchCtrl').DatazgsAA;
					console.log('Datazgs------------');
					console.log(Datazgs);
					//判断该潜在分公司是否存在潜在总部
					var flag=false;
					try{
						for(var i=0;i<Datazgs.length;i++){
							if(Datazgs[i].Name==record.data.Name&&Datazgs[i].AccountStatus=='潜在'){
								flag=true;
							}
						};
					}catch(e){
					...
					if(flag){
						//显示按钮
						document.getElementById("custominfo_new_id_TJSQTwo").style.display='block';
					}
				}else if(record.data.AccountStatus=='有效'){
			...
			//list滑动删除
			obj.getAddSliedDelete('custominfo_new_id_listV','OpportunityManagement.CustomerInformation_New.CustomInfoCtrl');
