    	...
    	if(num==6){
    		...
    		var getResult1=function(res){
    			...
    			if(res.data.length==0){
    				Ext.Msg.alert("温馨提示","您查询的数据量过大或没有匹配的结果，请增加查询条件及检查条件是否有误!");
    				return;
    			};
...
//        			cc.log(JSON.stringify(resDate));
    			...
    			/*var html='<table border=0 width=100% style="color:#666" class="textf">'+
                '  <tr>'+
    			...
    			var html='<table border=0 width=100% style="color:#666" class="textf">'+
                '  <tr>'+
                '     <td width=80%>{contract_no}(合同编号)</td>'+
                '     <td width=20%></td>'+
                '  </tr>'+
                '  <tr>'+
                '      <td width=100%>{contract_type_name}    {final_use_unit}    {customer_number}</td>'+
                '  </tr>'+
                '</table>';
    			Ext.getCmp('contractList_list').setItemTpl(html);
