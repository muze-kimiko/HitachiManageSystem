...
Ext.define("HelcPAD.controller.ApplicationController", {
	...
	/*添右滑删除按钮*/
	getAddSliedDelete:function(ListId,CtrlName){
		//作业项目list 左滑删除
                ...
                buttons:[{
                	...
                	listeners: {
                		tap: function(button, e){
                			//删除
                			object.getController(CtrlName).Delete='删除';
                            //用于回退，防止单击会调用list的方法
                            //e.stopPropagation();
                            //return false;
                		},
                	...
                	handler: function(button, e) {
                		List.fireEvent('hide'); // 隐藏列表的滑动按钮组
                	}
            },]
        });
	},
...
//				Ext.getCmp(id).focus();
	...
	//值列表公共方法 xcx 17-6-26
	//值类型，下拉控件ID,是否要请选择
	publicZLB:function(Type,Name,Flag){
		...
		ssdd+=']';
		Ext.getCmp(Name).setOptions(eval(ssdd));
	},
	
	//由上一级获得下一级的 值列表公共方法 xcx 17-6-26
	//条件一（上一级的值）,条件二（下拉列表类型）。下拉控件ID
	publicZLB_Two:function(TypeOne,TypeTwo,Name){
		var DataCs=Ext.data.StoreManager.get('CxAppLovVStore');
		if(!DataCs){
			DataCs=Ext.create('HelcPAD.store.CxAppLovVStore');
		};
		...
		for(var i=0;i<num;i++){//获取选择的数据在数据仓中的位置
			if(TypeOne==DataCs.getAt(i).get('PAR_LIS_VAL')&&TypeTwo==DataCs.getAt(i).get('TYPE')){
				JLidData[JLid]=i
				JLid++;
			};
		};
		...
		if(JLid==0){
			...
			for(var j=0;j<JLidData.length;j++){
				ssdd+="{'value':'"+DataCs.getAt(JLidData[j]).get('LIS_VAL')+"','text':'"+DataCs.getAt(JLidData[j]).get('LIS_VAL')+"'},";			
			}
		};
		ssdd+=']';
		Ext.getCmp(Name).setOptions(eval(ssdd));
				...
				onSuccess : function (result) {
					...
					if (200 == httpStatusCode) {
						...
						try{
							...
							if(num==1){
								try{
									var Msg=result.invocationResult.ContractListQueryByExample_Output.ErrMsg;
									var dp='存在的行比可返回的多。请细化您的查询以便产生较少的行';
									if(Msg.indexOf(dp)>0){
...
//		        	  Ext.Msg.alert('提示',"失败！");
					...
					if (200 == httpStatusCode) {
						...
						if (true == isSuccessful) {
							try{
								...
								//var msg=result.invocationResult
								Ext.Msg.alert("温馨提示","申请失败");
							};
							console.log("查询结果3-----------");
							console.log(data);
						} else {
	...
	//客户资料   提交申请    之      保存
	connectServer_custom_BC:function(flag,fn,Trim){
		if(flag){			
			myLoading.show();
		}
		var invocationData={
	...
	//客户资料   之提交申请
	connectServer_custom_TJSQ:function(flag,fn,Trim){
		if(flag){
			myLoading.hide();			
		}
		var invocationData={
	...
	//添加附件
	connectServer_custom_FJ_SC:function(flag,fn,obj,trim){
		if(flag){
			myLoading.show();			
		}
		var invocationData={
	...
	//我的商机    列表数据刷新
	getProjectSearchListRefresh:function(){
		console.log('我的商机    列表数据刷新');
		var statement = "[Opportunity.Oppty Type] = '设备商机'  and  EXISTS([Opportunity.Sales Login Name] = '"+userID+"' )";
		var param = {
				NewQuery:true,
				userID:userID,
