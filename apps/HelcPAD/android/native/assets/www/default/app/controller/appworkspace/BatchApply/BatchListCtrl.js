	...
	config:{
		control:{
		    ...
		    'list#batchList_id_list':{
		    	itemsingletap:'batchList_id_list'
			},
		...
		if(e.target.id=='ReasonCommentID'){
			...
			if(record.data.BILL_TYPE_NAME=='特殊排产发货申请流程'){
				...
				var getResult =function(result){
					...
					/*if(json.specialbill_head_line_list[0].special_line.length==undefined){
						console.log('test','ok');
					};*/
					//Ext.Msg.alert('温馨提示',json);
					var num1=json.specialbill_head_line_list.length;
					//验证是否有数据
					if(num1==0){
						...
						for(var i=0;i<num1;i++){
							var STA_MEAN=json.specialbill_head_line_list[i].special_line.STATUS_MEAN;
							//console.log('STA_MEAN',STA_MEAN);
							if(STA_MEAN=='行审批中'){
								var data1=json.specialbill_head_line_list[i].special_line;
								Ext.Msg.alert("<font size='3'>申请原因及说明</font>","原因："+data1.APPLY_REASON+"<br>说明："+data1.APPLY_COMMENTS);
								return;
							};
						};	
					};
					...
					/*
					var data=json.specialbill_head_line_list[0].special_line;
					var r=data.APPLY_REASON;
					var c=data.APPLY_COMMENTS;

					Ext.Msg.alert("<font size='3'>申请原因及说明</font>","原因："+r+"<br>说明："+c);
					return;
					*/
				};
...
//		}
	...
	tc_SpecialBatchApply:function(obj,result){
		...
		obj.NextView('specialBatchApply_id','HelcPAD.view.appworkspace.BatchApply.SpecialBatchApply');
		//cc.log('--------------result');
		//cc.log(result);
		//cc.log('--------------vv');
		//cc.log(vv);
