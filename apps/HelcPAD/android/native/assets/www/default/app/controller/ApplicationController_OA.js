...
/* JavaScript content from app/controller/ApplicationController_OA.js in folder common */
Ext.define("HelcPAD.controller.ApplicationController_OA", {
...
//		this.DestroyPublicId();
	...
	//返回：改
	showBackViewG : function(id, name) {
...
function initDate1(id,title){
	console.log('来自OA--------');
	need_text_id=id;
	var aimField = Ext.getCmp(id);
	var elementType = 'ext';
	if(!aimField){
		elementType = 'dom';
		aimField = document.getElementById(need_text_id);
	}	
	if(!picker_show1){
		picker_show1=Ext.Viewport.add({
			  xtype: 'datepicker',  
              ...
              toolbar:{
            	  ...
            	  items:[ 
                  {
	                   ...
	                   listeners:{
		                   tap:function(){
		                   	   aimField = elementType=='ext'?Ext.getCmp(need_text_id):document.getElementById(need_text_id);
		                   	   if(elementType=='ext')
		                   		   aimField.setValue('');
		                   	   else
		                   		   aimField.value = '';
             ...
             listeners:{
            	 change:function(obk,values,eOpts){
            	 	 aimField = elementType=='ext'?Ext.getCmp(need_text_id):document.getElementById(need_text_id);
                 	 if(elementType=='ext'){
                 	 	if(aimField.getValue()==Ext.Date.format(values,'Y-m-d')){
                 	 		aimField.setValue(Ext.Date.format(values,'Y-m-d')+' ');
                 	 	}else{
                 	 		aimField.setValue(Ext.Date.format(values,'Y-m-d'));
                 	 	}
                 	 }else{
            	 	 	if(aimField.value==Ext.Date.format(values,'Y-m-d')){
            	 	 		aimField.value=Ext.Date.format(values,'Y-m-d')+' ';
                 	 	}else{
                 	 		aimField.value=Ext.Date.format(values,'Y-m-d');
                 	 	}
                 	 }
...
//                	},
                 show:function(){
                	 cc.log()
                	 aimField = elementType=='ext'?Ext.getCmp(need_text_id):document.getElementById(need_text_id);
                	 var value=elementType=='ext'?aimField.getValue():aimField.value;
                	 if(value==''||value==null||typeof(value)=='undefined'||value=='点击文本设置时间'){
                		 picker_show1.setValue(new Date());
                	 }else{
                		 if(value.indexOf!=-1){
                		 	 value = value.split('-');
                		 	 picker_show1.setValue(new Date(value[0],(parseInt(value[1])-1)+'',value[2]));
                		 }else
                			 picker_show1.setValue(new Date());
                		 }
                	 }
...
function initDateKH(me){
	if(!picker_show1){
		picker_show1=Ext.Viewport.add({
			  ...
			  //minDate:Ext.Date.add(new Date(),Ext.Date.YEAR,-5),
			  //maxDate:Ext.Date.add(new Date(),Ext.Date.YEAR,10),
			  value: new Date(),
			  value:me.getValue()!=''?new Date(me.getValue()):new Date(),
			  format:'Y-m-d',
              doneButton: '完成',
              cancelButton: '取消',
              toolbar:{
             ...
             listeners:{
            	 change:function(obk,values,eOpts){
            		 try{
            			 cc.log(Ext.Date.format(values,'Y-m-d'));
            			 console.log(values);
            			 me.setValue(Ext.Date.format(values,'Y-m-d'));            			 
            		 }catch(e){
            			alert(e); 
            		 };
                 },
...
//                	},
                 show:function(){
                	 if(me.getValue()!=''){
                		 //注意为时间控件插入的时间格式 new Date()
                		 picker_show1.setValue(new Date(me.getValue()));    
                	 }
                 }
             }
		});
...
var picker_show;
//var need_text_id=null;
function initDateTTO(id,title,format,initDateCallback){
	need_text_id=id;
	var sOrder;
	switch(format){
	case "Y-m-d":
		sOrder = ['year','month','day'];
		break;
	case "Y-m":
		sOrder = ['year','month'];
		break;
	...
	picker_show=Ext.Viewport.add({
		...
		listeners:{
			change:function(obk,values,eOpts){
				if(Ext.getCmp(need_text_id).config.xtype == 'button'){
					if(Ext.getCmp(need_text_id).getText()==Ext.Date.format(values,format)){
						Ext.getCmp(need_text_id).setText(Ext.Date.format(values,format)+'');
					}else{
						Ext.getCmp(need_text_id).setText(Ext.Date.format(values,format));
					}
				}else{
					if(Ext.getCmp(need_text_id).getValue()==Ext.Date.format(values,format)){
						Ext.getCmp(need_text_id).setValue(Ext.Date.format(values,format)+'');
					}else{
						Ext.getCmp(need_text_id).setValue(Ext.Date.format(values,format));
					}
				}
				//if(initDateCallback)initDateCallback();
			},
			show:function(){
				var value;
				if(Ext.getCmp(need_text_id).config.xtype == 'button'){
					value=Ext.getCmp(need_text_id).getText();
				}else{
					value=Ext.getCmp(need_text_id).getValue();
				}
				
				if(value==''||value==null||typeof(value)=='undefined'||value.indexOf('点击')>-1){
					picker_show.setValue(new Date());
				}else{
					picker_show.setValue(new Date(value));
				}
			}
		}
    });
		...
		for (var x = 0; x < _next.length; x++) {
			...
			if (_next[x].id == to) {
				...
				if (assigntype == '4') {
					...
					age = "{assigntype:'" + assigntype + "',assignto:'"
							+ OA_df_usernames + "' ,piid:'" + acti.piid
							+ "', anyflag:'" + anyflag + "', multflag:'"
							+ multflag + "', multqty : '" + multqty + "'}";
				} else if(assigntype == '7'){
					age = "{assigntype:'" + assigntype + "',assignto:'"
							+ Ext.getCmp(assignto).getValue().split("*")[0] + "' ,piid:'"
							+ acti.piid + "', anyflag:'" + anyflag
							+ "', multflag:'" + multflag + "', multqty : '"
							+ multqty + "'}";
				}
...
//							+ encodeURI(jsonusertmpe.assignto);
				} else if(assigntype == '5' || assigntype == '6' || assigntype=='7'){
					var ygbh_String = "00000000"+userid;
					var OA_ygbh = ygbh_String.substring(ygbh_String.length-8,ygbh_String.length);
					age = '/unioa/processtrace.nsf/getleaderbycs1?openagent&user='+OA_ygbh+'&piid='+ jsonusertmpe.piid;
				} else {//人员设置为空时
