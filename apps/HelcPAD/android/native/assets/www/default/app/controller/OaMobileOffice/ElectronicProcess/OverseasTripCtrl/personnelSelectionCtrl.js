...
/* JavaScript content from app/controller/OaMobileOffice/ElectronicProcess/OverseasTripCtrl/personnelSelectionCtrl.js in folder common */
/**
 * 起草流程-人员选择
 */
Ext.define('HelcPAD.controller.OaMobileOffice.ElectronicProcess.OverseasTripCtrl.personnelSelectionCtrl',{
	extend:'HelcPAD.controller.ApplicationController_OA',
	id:'qc_personnelSelectionCtrl_ID',
	config:{
		control:{
			'button#nqc_returnApprovaOpinion_ID':{
				tap:'qc_personnelSelection',
			 },
			 'list#nqc_Camdodates_ID':{
				 itemtap:'qc_Camdodates',
			 },
			 'button#nqc_CompleteChoice_ID':{
				 tap:'qc_CompleteChoice',
			 },
			 'button#nqx_inquire_ID':{
				 tap:'qc_staffQuery',
			 }
		}
	},	
	...
	qc_personnelSelection : function(obj, e, eOpts){
		this.showBackView('qc_approvalOpinion_ID','HelcPAD.view.OaMobileOffice.ElectronicProcess.OverseasTrip.approvalOpinion');
		choosePerson = [];
...
//		}
	...
	qc_CompleteChoice : function(obj, e, eOpts){
		...
		var persons = [];
		var cs = 0;
		var store = this.getStore('personnelSelectionS','HelcPAD.store.OaMobileOffice.ElectronicProcess.OverseasTripS.approvalOpinionS');
...
//			var store = this.getStore('personnelSelectionS','HelcOA.store.startTheProcess.DailyOffice.Idea.personnelSelectionS');
	...
	qc_staffQuery : function(obj, e, eOpts){
		...
		if(!store){
			store=Ext.create("HelcPAD.store.OaMobileOffice.ElectronicProcess.OverseasTripS.qyeryListStore");
		}
		store.setData([]);
		...
		this.NextView('qc_qyeryList_id','HelcPAD.view.OaMobileOffice.ElectronicProcess.OverseasTrip.qyeryList');
	},
