...
/* JavaScript content from app/controller/OpportunityManagement/Project_New/BusinessAttachmentCtrl.js in folder common */

//获取图片转换Base64位字符串
var SrcDirs=[];

Ext.define('HelcPAD.controller.OpportunityManagement.Project_New.BusinessAttachmentCtrl',{
	...
	businessattachment_listV:function(list, index, target, record, e, eOpts){
		var obj  = this;
    	var store=obj.getStore('PictureStore','HelcPAD.model.OpportunityManagement.Project_New.OpptyPictureModel');
	...
	BusinessAttachment_new_id_PSZP:function(){
    	...
    	navigator.camera.getPicture(
				function(data){
					...
					var length=picture_list.length;
					Ext.getCmp('businessattachment_listV').setHeight(length==0?0:(length+1)*35);
					var store=obj.getStore('PictureStore','HelcPAD..OpportunityManagement.Project_New.PictureModel');
					store.setData(picture_list);
				},
				function(e){
					WL.Toast.show("取消选择图片");
	...
	BusinessAttachment_new_id_FH:function(){
		...
		obj.pictures = [];
		picture_list = [];
		var store=obj.getStore('OpptyPictureStore','HelcPAD.store.OpportunityManagement.Project_New.OpptyPictureStore');
		store.setData([]);
	},
	...
	toInit:function(){
		...
		var getResult = function(result){
			...
			if(!store)
				store = Ext.create('HelcPAD.store.OpportunityManagement.Project_New.OpptyPictureStore');
	...
	refreshPic:function(ctrl){
		...
		var obj = ctrl;//.getApplication().getController('HelcPAD.controller.OpportunityManagement.Project_New.BusinessAttachmentCtrl');
		var businessattchmentOpportunity = Ext.getCmp('businessattchmentOpportunity').getData();
		for(var i=0;i<picture_list.length;i++){
    		obj.pictures[i] = {
    				AttachmentType:'',
