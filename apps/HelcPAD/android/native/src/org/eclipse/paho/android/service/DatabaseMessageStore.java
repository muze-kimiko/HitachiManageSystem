...
public class DatabaseMessageStore implements MessageStore {
	...
	public String storeArrived(String clientHandle, String topic,
		...
		values.put(MqttServiceConstants.PAYLOAD, message.toString());
		...
		Log.d(TAG, clientHandle + ", Arrived count = " + count);
	    ...
	    try{
	       if(message.toString().contains("{")) {
	           ...
	           subject = jo_msg.optString("subject");
	           /*subject = new String(Base64.decode(subject),"GB2312");*/
	           subject = new String(Base64.decode(subject),"GB2312");
	           
	           jo_msg.put("message", msg.replace("nl", "<br/>"));
	           //jo_msg.put("title", tle.replace("nl", "<br/>"));
	           jo_msg.put("subject", subject.replace("nl", "<br/>"));
	          }
	      }catch (Exception e){
		...
		switch (apptype) {
		case Pmsg:
			doNotification(message.toString());
...
//			service.showNotificationWithSound(message.toString());
	 ...
	 */
	private void doNotification(String tmpmsg) {
		traceHandler.traceDebug(TAG, "---maintainNotification{" + tmpmsg + "}");
//		String statusText = (count == 0) ? null
//				: (count + " messages outstanding");
//		service.showNotification(tmpmsg);
		service.showNotificationWithSound(tmpmsg);
	}
