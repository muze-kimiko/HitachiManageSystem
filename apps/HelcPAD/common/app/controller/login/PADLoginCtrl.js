			...
			if(typeof(res)=="undefined"){
				...
				if(xmlDoc.getElementsByTagName("Msg")[0].hasChildNodes()){
					...
					if (Ext.os.is.iOS) {
					    ...
					    options.parameters = {
					    	//测试
//					        j_username : 'T'+userIDeight,
//					        j_password : 'T'+userIDeight
					        //正式
...
//初始化公共的JSONStore
function CSHJsonStoreGG(){

	var MaintainList=WL.JSONStore.get('OnlyFries');
	var query={tcode:'Name'};
	var options={
		exacte:true,//默认
		limit:200,//查询最大条数
	};
	MaintainList.find(query,options).then(function(arrayResults){
		//判断是否存在记录用户
		if(arrayResults==''){
			var Data=userIDeight;
			var Maintxml={tcode:'Name',XX:Data,};
			MaintainList.add(Maintxml).then(function(){
				cc.log('成功添加登陆人');
			}).fail(function(errorObject){
				WL.Toast.show("添加登陆人失败！");
			});
		}else{
			//登陆人记录存在，判断是否和当前登陆人相同
			var Data=arrayResults[0].json.XX;
			console.log('当前用户:'+userIDeight+'  上一个登陆用户:'+Data);
			//如果不等，退出订阅
			if(Data!=userIDeight){
				console.log('退出上一用户订阅');
				//取消订阅
		  	 	if (Ext.os.is.Android) {
					unsubscribeUser(Data);
				};
		
				//删除登陆人记录，添加新的登陆人，然后订阅
				MaintainList.remove(query,options).then(function(){
					...
					MaintainList.add(Maintxml).then(function(){
						...
						cc.log('登陆人信息添加失败！');
						WL.Toast.show("登陆人信息添加失败！");
					});
				}).fail(function(){
					cc.log('登陆人信删除失败');
					WL.Toast.show("登陆人信删除失败");
				});
			}
		};
		
	}).fail(function(errorObject){
		Ext.Msg.alert(errorObject);
		WL.Toast.show("检查上一次，登陆用户人员失败！");
	});	
...
function CSHJsonStore(obj){
	...
	collections['OnlyFries']={};
	collections['OnlyFries'].searchFields={tcode:'string',tid:'string',stext:'string',ZLB:'string',ZXTIME:'string',XX:'string'};
	...
	WL.JSONStore.init(collections).then(function(){
		...
		CSHJsonStoreGG();
		//测试用，可删除
		//var tt={"subject":"您有新的中标信息3","message":"订单号0008已中标","sendtime":"2016-08-03 16:15:38",rowID:"111"};
		//TXlist(tt);
...
//获取提醒传递过来的数据
//先查询JSONStore中有没有数据
//mb=new MqttBean(0,"Hitachi/HelcBP/T00006405/BP01","{\"sendtime\":\"2017-07-26 18:10:08\",\"rowid\":\"1-RCCOJ\",\"message\":\"ycy7+qGwQkEwMF9BSDEyXzAwMDAwMbnj1t3I1bH1v8a8vLei1bnT0M/euavLvsSjxOK5pLrFobG55raoyrG85MTazrTN6rPJycy7+te0zKyx5Lj8o6zH67ywyrG4+r34\",\"subject\":\"ycy7+szh0NHJ6NbDzOHQ0Q==\"}", 0);
function TXlist(msg){
	cc.log('--------------11');
	cc.log(msg);
	//是否有消息判断
	...
	var xx={
			...
			subject:Trim(json.subject),   //标题
			message:Trim(json.message),    //内容 		
			rowid:json.rowid  //商机ID
	};
	function Trim(str){
		return str.replace(/\s+/g,'');       
	}
	var MaintainList=WL.JSONStore.get('OnlyFries');
	var query={tcode:'TX',tid:collectionName};
	var options={
		exacte:true,//默认
		limit:200,//查询最大条数
	};
	MaintainList.find(query,options).then(function(arrayResults){
		...
		if(arrayResults==''){
			var Data=[];
			Data[0]=xx;
			var Maintxml={tcode:'TX',XX:Data,tid:collectionName};
			MaintainList.add(Maintxml).then(function(){
				cc.log('提醒消息添加成功！');
