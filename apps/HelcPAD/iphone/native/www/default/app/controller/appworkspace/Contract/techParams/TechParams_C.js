...
/* JavaScript content from app/controller/appworkspace/Contract/techParams/TechParams_C.js in folder common */
Ext.define('HelcPAD.controller.appworkspace.Contract.techParams.TechParams_C',{
	...
	config:{
		...
		control:{
			...
			//返回 合同行资料页面
			"button#techParams_Search_V_FH":{
				tap:'techParams_Search_V_FH'
			},
		}
	},
	
	techParams_Search_V_FH:function(){
		this.showBackView('contracthang_id','HelcPAD.view.appworkspace.Contract.ContractHang');
	},

	//点击list进入具体楼层
	TechParamsList : function(obj,index,target,record,e,eOpts){
		var obj = this;
		obj.NextView('techParams_Wllc_V','HelcPAD.view.appworkspace.Contract.techParams.TechParams_Wllc_V');
		var WL_TPS=WL.JSONStore.get(collectionName);
		var TechParamsStore = obj.getStore("TechParamsStore","HelcPAD.store.appworkspace.Contract.techParams.TechParamsStore");
	...
	backToTechParams : function(){
		this.showBackView('techParams_Search_V','HelcPAD.view.appworkspace.Contract.techParams.TechParams_Search_V');
	},
	...
	btn_techParams_Search : function(){
		var obj = this;
		var elevator_number = Ext.getCmp('CHang_AssetNumber').getValue();
		...
		if(elevator_number ==""){
			...
			var getResult=function(res){
				if(!res.item){
					...
					Ext.Msg.alert('提示',"找不到该工号的技术参数！");
				}else{
					
					obj.NextView('techParams_Search_V','HelcPAD.view.appworkspace.Contract.techParams.TechParams_Search_V');
					Ext.getCmp('formpanel_FT').setHidden(true);
					Ext.getCmp('formpanel_ZT').setHidden(true);
