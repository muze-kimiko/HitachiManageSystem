...
Ext.define('HelcPAD.controller.login.PADMainCtrl', {
	...
	csyong:function(obj, index, target, record, e, eOpts){
		...
		if(text=='申请分批单'){
			//申请分批单测试完美成功，但是没有正式数据库IP,所以该功能不予开放   xcx  2016-9-6  
	...
	appWorkSpace:function(obj, index, target, record, e, eOpts){
		...
		if(text == '客户管理'){
			//obj.NextView('customsearch_id','HelcPAD.view.OpportunityManagement.CustomerInformation.CustomSearch');
			...
			ssdd+=']';
			Ext.getCmp('customSearch_new_id_dkhmc').setOptions(eval(ssdd));
		}else if(text == '我的商机'){
