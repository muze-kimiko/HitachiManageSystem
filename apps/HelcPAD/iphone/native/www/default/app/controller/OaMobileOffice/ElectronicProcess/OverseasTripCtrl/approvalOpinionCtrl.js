...
/* JavaScript content from app/controller/OaMobileOffice/ElectronicProcess/OverseasTripCtrl/approvalOpinionCtrl.js in folder common */
/**
 * 审批意见
 */
Ext.define('HelcPAD.controller.OaMobileOffice.ElectronicProcess.OverseasTripCtrl.approvalOpinionCtrl',{
	extend:'HelcPAD.controller.ApplicationController_OA',
	id:'nqc_approvalOpinionCtrl_ID',
	config:{
		control:{
			//提交用印申请
			...
			'button#nqc_returnTravelRequest_ID':{
				tap:'qc_returnTravelRequest',
			 },
			 'button#nqc_submitTravelRequest_ID':{
				tap:'qc_submitTravelRequest',
			 },
			 "list#nqc_reutingList_ID":{
				itemtap:'qc_select'
			 },
		}
	},
	...
	qc_select : function(obj, index, target, record, e, eOpts){
		...
		function aa(assign,vdata,idx){
			...
			if(!store){
				store=Ext.create("HelcPAD.store.OaMobileOffice.ElectronicProcess.OverseasTripS.approvalOpinionS");
			}
			...
			if(typeof(personnelList["QC_"+vdata.name])!=undefined && personnelList["QC_"+vdata.name]!=null && personnelList["QC_"+vdata.name]=="QC_"+vdata.name){
				obj_this.showBackView('qc_personnelSelection_id','HelcPAD.view.OaMobileOffice.ElectronicProcess.OverseasTrip.personnelSelection');
				...
				function getResult(result) {
					...
					obj_this.NextView('qc_personnelSelection_id','HelcPAD.view.OaMobileOffice.ElectronicProcess.OverseasTrip.personnelSelection');
					//默认选中带出来的所有人
					if(userSolist.length!=0){
						var sbenoCheck=document.getElementsByName('p_judge_color');
						for(var nb=0;nb<userSolist.length;nb++){
							sbenoCheck[nb].className='p_judge_box_clicked';
							var personnelSelectionS = obj_this.getStore('personnelSelectionS','HelcPAD.store.OaMobileOffice.ElectronicProcess.OverseasTripS.personnelSelectionS');
		...
		var backFunc=function (obj2){
			Ext.Msg.confirm('','确定提交？',function(btn){
				if (btn == 'yes'){
					...
					obj2.QC_connectServer(getResult3,content,1);
