...
/* JavaScript content from app/controller/BPMProcess/BPMProcessCtrl.js in folder common */
 Ext.define('HelcOA.controller.BPMProcess.BPMProcessCtrl',{
	...
	BPMP_arrow_up : function(){
		...
		var getResult=function(res){
			...
			for(var i=0;i<data.length;i++){
				if(data[i].flag=='1'){
					...
					data[i].flag='已结束:否决';
				}else if(data[i].flag==''||data[i].flag==' '||data[i].flag==null||typeof(data[i].flag)=="undefined"){
					data[i].flag='-';
				}else{
					data[i].flag='其他';
				}
		...
		params.parameters = [fenlei,proc_name,cd_flag,subject,billno,df_man,page,start,limit,userkey];
