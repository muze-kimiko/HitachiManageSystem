...
/* JavaScript content from app/controller/startTheProcess/DailyOffice/Idea/approvalOpinionCtrl.js in folder common */
/**
 * 审批意见
 */
Ext.define('HelcOA.controller.startTheProcess.DailyOffice.Idea.approvalOpinionCtrl',{
	...
	qc_select : function(obj, index, target, record, e, eOpts){
		...
		function aa(assign,vdata,idx){
			...
			if(!store){
				store=Ext.create("HelcOA.store.startTheProcess.DailyOffice.Idea.personnelSelectionS");
			}
			...
			if(typeof(personnelList["QC_"+vdata.name])!=undefined && personnelList["QC_"+vdata.name]!=null && personnelList["QC_"+vdata.name]=="QC_"+vdata.name){
				...
				function getResult(result) {
					...
					obj_this.NextView('qc_personnelSelection_id','HelcOA.view.startTheProcess.DailyOffice.Idea.personnelSelection');
					//默认选中带出来的所有人
					if(userSolist.length!=0){
						var sbenoCheck=document.getElementsByName('p_judge_color');
						for(var nb=0;nb<userSolist.length;nb++){
							sbenoCheck[nb].className='p_judge_box_clicked';
							var personnelSelectionS = obj_this.getStore('personnelSelectionS','HelcOA.store.startTheProcess.DailyOffice.Idea.personnelSelectionS');
