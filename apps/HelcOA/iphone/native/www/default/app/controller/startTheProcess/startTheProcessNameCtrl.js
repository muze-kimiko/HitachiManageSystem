...
/* JavaScript content from app/controller/startTheProcess/startTheProcessNameCtrl.js in folder common */
Ext
		.define(
				'HelcOA.controller.startTheProcess.startTheProcessNameCtrl',
				{
					...
					init1 : function(obj, index, target, record, e, eOpts) {
						...
						if (bpdname == '工作联络书') {
							...
							otherfee.addListener('change', obj_this.FreeInfo,
									this, {});
							// var qystyle = Ext.getCmp('qystyle');
							// qystyle.addListener('change',obj_this.test22,this,{
							// });
							// 给预计小费添加总费用监听
							// for(var i=1;i<5;i++){
							// var fyxj =Ext.getCmp('fyxj'+i);
							// fyxj.addListener('change',obj_this.monenyChange,this,{
							// });
							// };
							// var button5 = Ext.getCmp('otherfee');
							// button5.addListener('blur',obj_this.yujiqitaMoneny,this,{});
							Ext.getCmp('qc_surface_ID').setTitle(
									record.data.bpdname);
						} else if (bpdname == '投资公司经理出差申请流程') {
							this
						...
						var getResult = function(res) {
							...
							var getResult2 = function(res2) {
								...
								if (bpdname == '公司发文流程') {
									...
									/*
									 * var jsonObj=eval("("+ res2.ovar +")");
									 * var jss=jsonObj.data; var jss2=jss.acti;
									 * var jss3=eval("("+ jss2 +")");
									 * console.log(JSON.stringify(jss3.node.name) +'
									 * =======U1'); var
									 * nameData=JSON.stringify(jss3.node.name);
									 * alert(nameData);
									 */
									// 判断
									/*
									 * var
									 * frm_dept_value=Ext.getCmp('dept').getValue();
